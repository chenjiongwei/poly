...
FROM(SELECT ROW_NUMBER() OVER (ORDER BY 延期月 DESC) AS 序号 ,
     ...
     FROM   (SELECT '华南公司' AS 公司名称 ,
                    ...
                    CASE WHEN DATEDIFF(YEAR, 实际供货完成日期, GETDATE()) = 0 THEN 本年计划供货面积 ELSE 0 END AS 本年已供货面积 ,
                    ...
                    CASE WHEN DATEDIFF(MONTH, 计划供货完成日期, GETDATE()) = 0 THEN 本年计划供货面积 ELSE 0 END AS 本月已供货面积 ,
