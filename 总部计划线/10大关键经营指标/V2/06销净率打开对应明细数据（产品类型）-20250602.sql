...
(
    ...
    GROUP BY projguid,
		 产品类型
) b ON a.projguid = b.projguid and a.产品类型 = b.产品类型
LEFT JOIN
(
    SELECT projguid,
		 产品类型,
    ...
    GROUP BY projguid,
		 产品类型
) bb ON a.projguid = bb.projguid and a.产品类型 = bb.产品类型
LEFT JOIN
(
    SELECT projguid,
		 产品类型,
    ...
    GROUP BY projguid,
		 产品类型
) c ON a.projguid = c.projguid and a.产品类型 = c.产品类型
...
(
    ...
    GROUP BY projguid,
		 产品类型
) d ON a.projguid = d.projguid and a.产品类型 = d.产品类型
LEFT JOIN
(
    SELECT projguid,
		 产品类型,
    ...
    GROUP BY projguid,
		 产品类型
) e ON a.projguid = e.projguid and a.产品类型 = e.产品类型
LEFT JOIN
(
    SELECT projguid,
		 产品类型,
    ...
    GROUP BY projguid,
		 产品类型
) f ON a.projguid = f.projguid and a.产品类型 = f.产品类型
LEFT JOIN 
(
    SELECT projguid,
		 产品类型,
    ...
    GROUP BY projguid,
		 产品类型
) sa ON a.projguid = sa.projguid and a.产品类型 = sa.产品类型
LEFT JOIN
(
    SELECT projguid,
		 产品类型,
    ...
    GROUP BY projguid,
		 产品类型
) sb ON a.projguid = sb.projguid and a.产品类型 = sb.产品类型;
...
SELECT f.projguid,
       ...
       END 是否统计
FROM #benzhou a
     LEFT JOIN #jjl j ON a.projguid = j.projguid and j.产品类型 = a.产品类型
     LEFT JOIN #ljqy24 l ON a.projguid = l.projguid and l.产品类型 = a.产品类型
     LEFT JOIN vmdm_projectflag f ON a.projguid = f.projguid
     LEFT JOIN #symj s ON a.projguid = s.projguid and s.ProductType = a.产品类型
ORDER BY f.平台公司,
         f.项目代码;
