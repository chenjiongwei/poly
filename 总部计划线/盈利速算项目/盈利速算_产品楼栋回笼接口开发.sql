USE [ERP25]
GO
/****** Object:  StoredProcedure [dbo].[usp_ylgh_cpldhl_part1]    Script Date: 2025/10/23 17:40:25 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROC [dbo].[usp_ylgh_cpldhl_part1](@var_projguid varchar(max), @tqrq DATETIME)
AS 
BEGIN
/*
产品楼栋回笼接口
取数范围：不含特殊业绩和合作业绩，原因是特殊业绩6个月很难完成回笼，数据铺排意义有限@20250909
...
取数规则：
(1) 取近6个月已回完款的房间
(2) 按照实际回款日期从签约日期计算出每个月的回款 ，由于存在房间多收退款，只统计收款单，来计算比例
(3) 基于每个月的回款数计算出 非贷款类、公积金、贷款类房款每个月的回款比例 ；非贷款类房款的负月份归集到第一个月
(4) 计算出已售未回部分的每个月的回款金额（含公积金、贷款类、非按揭）
(5) 非按揭取供款表的计划回款时间
(6) 获取到特殊业绩的回笼日期

遗留问题：
（1）非贷款类逾期的房款如何处理--调整供款表
（2）近3/6个月开盘的数据是不是会异常，负数归为
（3）已售未回部分怎么按月份铺排？历史部分怎么处理？
...
-- exec usp_ylgh_cpldhl_part1 'A862EDDC-0495-EF11-B3A5-F40270D39969',getdate();

select * from p_project where projname ='颂展花园-颂展花园';
declare @var_projguid varchar(max) = 'fbf40d1f-baae-ea11-80b8-0a94ef7517dd';
declare @tqrq DATETIME = getdate();
exec usp_ylgh_cpldhl_part1 @var_projguid,@tqrq
*/
...
LEFT JOIN (
      select tradeguid,sum(ISNULL(Ye, 0) - ISNULL(DsAmount, 0)) as Amount  
    from s_fee 
...
select 
    ...
    cast(sum(t.回笼金额)/10000.0 as decimal(18,6))  as 回笼金额,
    cast(sum(t.非贷款类已售未回款金额)/10000.0 as decimal(18,6))  as 非贷款类已售未回款金额,
    cast(sum(t.贷款类_公积金已售未回金额)/10000.0 as decimal(18,6))  as 贷款类_公积金已售未回金额,
    cast(sum(t.贷款类_按揭已售未回金额)/10000.0 as decimal(18,6))  as 贷款类_按揭已售未回金额,
    cast((sum(t.非贷款类已售未回款金额) + sum(t.贷款类_公积金已售未回金额) + sum(t.贷款类_按揭已售未回金额))/10000.0 as decimal(18,6)) as 已售未回款金额
from (
