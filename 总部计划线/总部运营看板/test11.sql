...
SELECT * 
FROM (
    SELECT * FROM #sumqy                      -- 总签约数据
    UNION  
    SELECT * FROM #sumqyfenlei               -- 分类签约数据(按产品类型和项目分类)
    UNION  
    SELECT * FROM #sumqybczhuanhua           -- BC赛道盘活转化数据
    UNION  
    SELECT * FROM #sumqydnhuoqu              -- 当年获取当年签约数据
    UNION  
    SELECT * FROM #sumqytotal                -- 现有可售资源合计
    UNION  
    SELECT * FROM #sumxjl 
    UNION  
    SELECT * FROM #LowXjlqy                   -- 销售净利率数据
    UNION  
    SELECT * FROM #sumqichuzaijiankg         -- 期初在建面积数据
    UNION  
    SELECT * FROM #sumxinkgdishang           -- 地上新开工面积数据
    UNION  
    SELECT * FROM #sumxinkgdishangchaifen    -- 地上新开工面积按获取年份分类数据
    UNION  
    SELECT * FROM #sumxinkg                  -- 新开工面积数据
    UNION  
    SELECT * FROM #sumxinkgchaifen           -- 新开工面积按获取年份分类数据
    UNION  
    SELECT * FROM #sumxinjungongbeian        -- 竣工面积数据
    UNION  
    SELECT * FROM #sumjf                     -- 交付套数数据
    UNION  
    SELECT * FROM #sumfeiyong                -- 营销费用数据
    UNION  
    SELECT * FROM #sumjfcp                   -- 操盘交付套数数据
) a
