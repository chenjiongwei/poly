...
SELECT  bu.buname AS [公司名称],
        ...
        mp.ConstructStatus AS [工程状态],-- 分期工程状态
       CASE WHEN EXISTS (
            SELECT 1 
            FROM p_project sub WITH(NOLOCK)
            INNER JOIN ERP25.dbo.mdm_project submp WITH(NOLOCK) ON submp.projguid = sub.ProjGUID
            WHERE sub.ParentCode = pp.ProjCode 
            AND sub.Level = 3
            AND submp.ConstructStatus <> '已完工'
        ) THEN '在建/拟建'
        ELSE '已完工' END AS [项目工程状态],
        flg.工程操盘方 AS [工程操盘方],
        flg.成本操盘方 AS [成本操盘方],
        ...
        ht.PayAmount AS [合同_累计付款登记],
        ht.NoPayAmount AS [合同_累计付款登记不含税],
        ht.JhfkAmountNoTax AS [合同_累计付款登记不含可抵扣税],
        ht.ApplyAmount AS [合同_累计付款申请金额],
        ht.balanceamount AS [合同_结算金额],
        -- 不含土地及三费
        ...
        htNotfee.PayAmountNotfee AS [合同_累计付款登记不含土地三费],
        htNotfee.PayAmount202502Notfee AS [合同_累计付款登记不含土地三费截止2月],
        htNotfee.NoPayAmountNotfee AS [合同_累计付款登记不含土地三费不含税],
        htNotfee.JhfkAmountNoTaxNotfee AS [合同_累计付款登记不含土地三费不含可抵扣税],
        htNotfee.ApplyAmountNotfee AS [合同_累计付款申请金额不含土地三费],
        htNotfee.balanceamountNotfee AS [合同_结算金额不含土地三费],
        ...
        cbview.cb_ApproveState AS [成本_产值月度回顾单据状态],
        cbview.ReviewDateMonth AS [成本_产值月度回顾最新发起月份],
        cbview.createon AS [成本_产值月度回顾最新发起人],
        jdview.jd_ApproveState AS [计划_月度形象进度单据状态],
        jdview.ReportDateMonth AS [计划_月度形象进度最新发起月份],
        jdview.UserName AS [计划_月度形象进度最新发起人],
        isnull(htXmpdljwccz.Xmpdljwccz,0) as [成本月度回顾_项目盘点累计完成产值],
        isnull(htXmpdljwccz.Ljyfkje,0) as [成本月度回顾_项目盘点累计应付款],
        isnull(htXmpdljwccz.Ljsfk,0) as [成本月度回顾_累计实付款],
        isnull(htXmpdljwccz.ydczwzfAmount,0) as [成本月度回顾_已达产值未支付金额],
        isnull(htXmpdljwccz.yfwsAmount,0) as [成本月度回顾_应付未付金额],
...
FROM    p_project p WITH(NOLOCK)
        ...
        OUTER APPLY (
            ...
            FROM (
                SELECT jd_OutValueView.ProjGUID,
                       jd_OutValueView.ReportDate, 
                       jd_OutValueView.ApproveState,
                       mu.UserName,
                       ROW_NUMBER() OVER (ORDER BY ReportDate DESC) as rn
                FROM jd_OutValueView WITH(NOLOCK)
                inner join myUser mu WITH(NOLOCK) on mu.userguid = jd_OutValueView.CreatedOn
        ...
        LEFT JOIN (
            SELECT  cbp.ProjGUID,
                    ...
                    sum(isnull(pay.PayAmount202502,0)) as PayAmount202502Notfee, -- 合同-累计付款登记（不含土地+三费）截止今年2月28日
