-- 清洗项目经营计划统计看板数据集
CREATE OR ALTER PROC usp_zb_jyjhtjkb_clean
AS
BEGIN
    -- 清洗投资假定条件数据集
    PRINT '>>>>>开始清洗投资假定条件数据集'
    EXEC usp_zb_jyjhtjkb_PositDiff;

    -- 清洗总收入指标
    PRINT '>>>>>开始清洗总收入指标数据集'
    EXEC usp_zb_jyjhtjkb_SaleIncome;

    -- 清洗货值指标
    PRINT '>>>>>开始清洗货值指标数据集'
    EXEC usp_zb_jyjhtjkb_SaleValue;

    -- 清洗总投资指标
    PRINT '>>>>>开始清洗总投资指标数据集'
    EXEC usp_zb_jyjhtjkb_TotalInvestment;

    -- 清洗利润指标
    PRINT '>>>>>开始清洗利润指标数据集'
    EXEC usp_zb_jyjhtjkb_Profit;

    -- 清洗现金流指标
    PRINT '>>>>>开始清洗现金流指标数据集'
    EXEC usp_zb_jyjhtjkb_CashFlow;

    -- 资产负债指标
    PRINT '>>>>>开始清洗资产负债指标数据集'
    EXEC usp_zb_jyjhtjkb_BalanceSheet;
END
...
-- 18409189-6E34-EF11-B3A4-F40270D39969 龙川臻悦
select
    projguid,
    case
        when projguid = '18409189-6E34-EF11-B3A4-F40270D39969' then
            '注：安徽公司龙川臻悦项目从最新一版盈利速算取数指标：'
            + '【总收入-动态版】、【住宅截止本月已售均价】、【除地价外直投-动态版】、【已发生除地价外直投】、【财务费用(单利)-动态版】、'
            + '【已发生财务费用】、【财务费用(单利)可售单方-截止本月】、【营销费用-动态版】、【已发生营销费用】、'
            + '【管理费用-动态版】、【已发生管理费用】、【总投资-动态版】、【已发生总投资-动态版】、'
            + '【增值税及附加-动态版】、【税前成本利润率-动态版】、【税前利润-动态版】、'
            + '【销售净利率-动态版】、【税后利润-动态版】、【税后现金利润-动态版】、'
            + '【全投资IRR-动态版】、【现金流回正时间-动态版】、【现金流回正时长-动态版】'
    end as 备注说明
from
    zb_jyjhtjkb_Profit
-- WHERE DATEDIFF(DAY, 清洗日期, ${qxDate}) = 0