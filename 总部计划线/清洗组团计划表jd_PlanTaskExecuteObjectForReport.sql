...
INTO #tr
FROM #t t
LEFT JOIN dbo.p_Project p1 WITH (NOLOCK) ON p1.ProjCode = t.ParentCode
LEFT JOIN p_HkbProjectWork p2 WITH (NOLOCK) ON p2.ProjGUID = p1.ProjGUID
LEFT JOIN ERP25.dbo.mdm_Project p3 WITH (NOLOCK) ON p3.ProjGUID = p2.RefernceProjectGUID
LEFT JOIN ERP25.dbo.mdm_LbProject lb WITH (NOLOCK) ON lb.projGUID = p3.ProjGUID AND lb.LbProject = 'tgid'
LEFT JOIN ERP25.dbo.myBizParamOption mb WITH (NOLOCK) ON p3.XMSSCSGSGUID = mb.ParamGUID
LEFT JOIN ERP25.dbo.myBizParamOption mb1 WITH (NOLOCK) ON mb.ParentCode = mb1.ParamCode AND mb1.ParamName = 'mdm_XMSSCSGS' AND mb.ScopeGUID = mb1.ScopeGUID
LEFT JOIN (
    SELECT projGUID, MAX(LbProjectValue) AS LbProjectValue
    FROM ERP25..mdm_LbProject WITH (NOLOCK)
    WHERE LbProject = 'sfnrtj'
    GROUP BY projGUID
) con ON con.projguid = t.ProjGUID
INNER JOIN dbo.myBusinessUnit bu WITH (NOLOCK) ON bu.BUGUID = t.BUGUID
LEFT JOIN (
    SELECT DISTINCT d.ObjectID, tg.TgType
    FROM (
    ...
    LEFT JOIN MyCost_Erp352.dbo.jd_ProjectPlanTaskExecute f WITH (NOLOCK) ON f.PlanID = tg.PlanID AND f.Level = '1'
    LEFT JOIN MyCost_Erp352.dbo.jd_ProjectPlanExecute d WITH (NOLOCK) ON d.ID = f.PlanID AND d.PlanType = '103'
    WHERE tg.num = 1 AND d.ObjectID IS NOT NULL
) cyj ON cyj.ObjectID = t.BuildGUID
LEFT JOIN (
    SELECT DISTINCT a.*,
           CASE WHEN b.BuildGUID IS NULL THEN '毛坯' ELSE '装修' END zxbz,
           c.UpNum,
           d.DownNum
    FROM (
        SELECT pw.BuildGUID,
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE md_Project.ApproveState = '已审核' AND ISNULL(InValidReason, '') <> '补录'
            ) t
            WHERE t.rowno = 1
        ) a
        LEFT JOIN p_HkbBiddingSectionWork sw WITH (NOLOCK) ON sw.projguid = a.projguid
        LEFT JOIN p_HkbBiddingBuildingWork pw WITH (NOLOCK) ON pw.BidGUID = sw.BidGUID
        LEFT JOIN dbo.p_HkbBiddingBuilding2BuildingWork bw WITH (NOLOCK) ON bw.BudGUID = pw.BuildGUID
        LEFT JOIN dbo.md_GCBuild gc WITH (NOLOCK) ON gc.BldGUID = bw.BuildingGUID AND a.VersionGUID = gc.VersionGUID
        LEFT JOIN dbo.md_ProductBuild pb WITH (NOLOCK) ON pb.BldGUID = gc.BldGUID AND pb.VersionGUID = a.VersionGUID
        LEFT JOIN dbo.md_Product pd WITH (NOLOCK) ON pd.ProductGUID = pb.ProductGUID AND pd.VersionGUID = a.VersionGUID
        WHERE a.Level = 3
    ) a
    LEFT JOIN (
        SELECT pw.BuildGUID
        ...
        LEFT JOIN (
            SELECT ProductBuildGUID,
                   ...
                   CASE WHEN pp.IsManualManageProductBld = 1 THEN a.IsSale ELSE e.IsSale END AS IsSale
            FROM MyCost_Erp352..md_ProductBuild a WITH (NOLOCK)
            LEFT JOIN MyCost_Erp352..md_Product e WITH (NOLOCK) ON e.ProductKeyGUID = a.ProductKeyGUID
            LEFT JOIN MyCost_Erp352..md_Project b WITH (NOLOCK) ON a.VersionGUID = b.VersionGUID
            LEFT JOIN (
                SELECT ProjGUID, IsManualManageProductBld
                FROM MyCost_Erp352..md_Project WITH (NOLOCK)
                WHERE Level = 2
                GROUP BY ProjGUID, IsManualManageProductBld
            ) pp ON pp.projguid = b.ParentProjGUID
        ) ww ON ww.ProductBuildGUID = pb.ProductBuildGUID AND ww.VersionGUID = a.VersionGUID
        LEFT JOIN dbo.md_Product pd WITH (NOLOCK) ON pd.ProductGUID = pb.ProductGUID AND pd.VersionGUID = a.VersionGUID
        WHERE a.Level = 3 AND pb.Zxbz = '装修' AND ww.IsSale = '是'
    ...
    LEFT JOIN (
        ...
        LEFT JOIN dbo.md_ProductBuild pb WITH (NOLOCK) ON pb.BldGUID = gc.BldGUID AND pb.VersionGUID = a.VersionGUID
        LEFT JOIN dbo.md_Product pd WITH (NOLOCK) ON pd.ProductGUID = pb.ProductGUID AND pd.VersionGUID = a.VersionGUID
        WHERE a.Level = 3 AND pw.BuildGUID IS NOT NULL
    ) d ON d.BuildGUID = a.BuildGUID AND d.xh = '1'
    WHERE a.xh = 1 AND c.xh = 1 AND a.BuildGUID IS NOT NULL
