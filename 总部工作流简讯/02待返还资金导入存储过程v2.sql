USE [dss]
GO
/****** Object:  StoredProcedure [dbo].[usp_nmap_F_平台公司待返还资金]    Script Date: 2025/6/4 10:40:18 ******/
SET ANSI_NULLS ON
GO
...
-- 创建存储过程
create   PROC [dbo].[usp_nmap_F_平台公司待返还资金V2]
    (
...
*/
  ...
  FROM nmap_F_平台公司待返还资金V2 
    ...
    WHERE   FILLDATAGUID = ( SELECT FILLDATAGUID
			...
			AND FILLHISTORYGUID IN (SELECT DISTINCT FILLHISTORYGUID FROM nmap_F_平台公司待返还资金V2
			WHERE ISNULL(项目名称,'') <> '')
    ORDER BY ENDDATE DESC;	
	...
	SELECT  
      ...
      ,isnull(d.[新排预计7月收回金额],a.[新排预计7月收回金额]) as [新排预计7月收回金额]
      ,isnull(d.[新排预计8月收回金额],a.[新排预计8月收回金额]) as [新排预计8月收回金额]
      ,isnull(d.[新排预计9月收回金额],a.[新排预计9月收回金额]) as [新排预计9月收回金额]
      ,isnull(d.[新排预计10月收回金额],a.[新排预计10月收回金额]) as [新排预计10月收回金额]
      ,isnull(d.[新排预计11月收回金额],a.[新排预计11月收回金额]) as [新排预计11月收回金额]
      ,isnull(d.[新排预计12月收回金额],a.[新排预计12月收回金额]) as [新排预计12月收回金额]
      ,isnull(d.[2026年计划盘活金额],a.[2026年计划盘活金额]) as [2026年计划盘活金额]
      ,isnull(d.[2027年计划盘活金额],a.[2027年计划盘活金额]) as [2027年计划盘活金额]
      ...
      ,''  as [预计本季度工作计划是否按节点达成]
      ,''  as [未按节点达成事项]
      ,''  as [本季度已收回金额（并表口径、单位万元）]
      ,''  as [全年累计收回任务（并表口径、单位万元）]
      ,'' as [全年累计收回金额（并表口径、单位万元）]
      ,'' as [全年累计收回逾期金额（并表口径、单位万元）]
      ,'' as [盘活资金比例]
      ,'' as [剩余全口径占压金额（单位万元）]
      ,'' as [剩余权益口径占压金额（单位万元）]
      ,'' as [剩余并表口径占压金额（单位万元]
	...
	-- 查询上一版的数据进行继承
	left join (
           select  distinct * from  nmap_F_平台公司待返还资金V2  
...
-- --删除旧数据  
    DELETE  FROM nmap_F_平台公司待返还资金V2
    WHERE   FillHistoryGUID = @FillHistoryGUID;  

	 -- 将数据插入到当前版本
	 INSERT INTO nmap_F_平台公司待返还资金V2
