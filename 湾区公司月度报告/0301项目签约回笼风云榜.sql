...
SELECT  convert(datetime, 清洗时间 ) as 清洗时间,
        ROW_NUMBER() OVER ( PARTITION BY 清洗时间,组织架构类型 ORDER BY(ISNULL(本月签约完成率, 0) * 0.5 + ISNULL(本月回笼完成率, 0) * 0.5) DESC) AS 倒序排序 ,
        ROW_NUMBER() OVER ( PARTITION BY 清洗时间,组织架构类型 ORDER BY(ISNULL(本月签约完成率, 0) * 0.5 + ISNULL(本月回笼完成率, 0) * 0.5)) AS 正序排序 ,
...
union ALL
SELECT  convert(datetime, 清洗时间 ) as 清洗时间,
        ROW_NUMBER() OVER ( PARTITION BY 清洗时间,组织架构类型,组织架构名称 ORDER BY(ISNULL(本月签约完成率, 0) * 0.5 + ISNULL(本月回笼完成率, 0) * 0.5) DESC) AS 倒序排序 ,
        ROW_NUMBER() OVER ( PARTITION BY 清洗时间,组织架构类型,组织架构名称 ORDER BY(ISNULL(本月签约完成率, 0) * 0.5 + ISNULL(本月回笼完成率, 0) * 0.5)) AS 正序排序 ,
       -- (ISNULL(本月签约完成率, 0) * 0.5 + ISNULL(本月回笼完成率, 0) * 0.5) AS 完成率排序 ,
        平台公司名称 ,
        平台公司GUID ,
        组织架构名称 ,
