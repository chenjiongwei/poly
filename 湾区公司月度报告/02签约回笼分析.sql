...
BEGIN
	...
	INNER JOIN highdata_prod.dbo.dw_s_WqBaseStatic_baseinfo base ON a.组织架构id = base.组织架构id and a.清洗时间id = base.清洗时间id
	...
	inner join #qxDate d on  DATEDIFF(day,a.清洗时间,d.清洗时间月末) =0
    WHERE   a.组织架构类型 = 3 AND  平台公司名称 = '湾区公司'
    GROUP BY GROUPING SETS((a.清洗时间,a.平台公司名称, a.平台公司GUID, base.区域 , base.存量增量), (a.清洗时间,a.平台公司名称,a.平台公司GUID,base.区域))  
	UNION ALL 
	--从清洗底表中取业态层级对应时间进度
     SELECT  a.清洗时间,
	        平台公司名称 ,
    ...
    inner join #qxDate d on  DATEDIFF(day,a.清洗时间,d.清洗时间月末) =0
    WHERE   a.组织架构类型 = 4 AND  平台公司名称 = '湾区公司'
    GROUP BY  a.清洗时间, 平台公司名称 ,base.区域,
            平台公司GUID , CASE WHEN  a.业态 IN  ('高级住宅','住宅','别墅') THEN  '住宅' 
			           WHEN a.业态 ='地下室/车库' THEN  '车位'
					  ELSE  '商办' END
    ...
    SELECT  清洗时间,
            ...
            CASE WHEN SUM(ISNULL(s.本年回笼任务, 0)) = 0 THEN 0 ELSE SUM(ISNULL(s.本年回笼金额, 0)) / SUM(ISNULL(s.本年回笼任务, 0))END AS 本年回笼完成率 
    FROM    #SaleTmp s
    GROUP BY GROUPING SETS((清洗时间,平台公司名称, 平台公司GUID, 区域,存量增量), (清洗时间,平台公司名称, 平台公司GUID,区域))
    ...
    FROM    #YtSaleTmp s
	GROUP BY  清洗时间,平台公司名称 ,
            平台公司GUID,区域, CASE WHEN  业态 IN  ('高级住宅','住宅','别墅') THEN  '住宅' 
			           WHEN 业态 ='地下室/车库' THEN  '车位'
					  ELSE  '商办' END  -- 统计各区域签约数据   
    ...
    SELECT  
	       convert(datetime, a.清洗时间 ) as 清洗时间,
			...
			a.本年回笼完成率
    FROM    #MonthSale a
            left  JOIN #lastMonthSale b ON a.平台公司GUID = b.平台公司GUID AND a.分析维度 = b.分析维度 and datediff(month,b.清洗时间,a.清洗时间) =1 and a.组织架构类型 = b.组织架构类型 and  a.组织架构名称 = b.组织架构名称
