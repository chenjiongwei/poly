...
select  
    ...
    1 as 组织架构类型,
    -- 本年开工相关指标
    sum(case when DATEDIFF(year,shi.实际开工计划完成时间,shi.清洗时间) = 0 then BuildArea end) as 本年计划开工面积,
    sum(case when DATEDIFF(year,shi.实际开工实际完成时间,shi.清洗时间) = 0 then BuildArea end) as 本年实际开工面积,
    case 
        when sum(case when DATEDIFF(year,实际开工计划完成时间,shi.清洗时间) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(year,实际开工实际完成时间,shi.清洗时间) = 0 then BuildArea end) /
             sum(case when DATEDIFF(year,实际开工计划完成时间,shi.清洗时间) = 0 then BuildArea end)
    end as 本年开工完成率,
    ...
    case
        when sum(case when DATEDIFF(month,实际开工计划完成时间,shi.清洗时间) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(month,实际开工实际完成时间,shi.清洗时间) = 0 then BuildArea end) /
             sum(case when DATEDIFF(month,实际开工计划完成时间,shi.清洗时间) = 0 then BuildArea end)
    end as 本月开工完成率,
    ...
    case
        when sum(case when DATEDIFF(year,竣工备案计划完成时间,shi.清洗时间) = 0 then BuildArea end) = 0 then 0
...
where bd.BUGUID = '248B1E17-AACB-E511-80B8-E41F13C51836' 
    and bd.BldType = '产品楼栋'
...
from data_wide_dws_mdm_Building bd
inner join dw_s_WqBaseStatic_ScheduleInfo shi on 组织架构类型 = 7 and bd.BuildingGUID = shi.组织架构ID
inner join dw_s_WqBaseStatic_BaseInfo bi on bi.组织架构id = shi.组织架构id and bi.清洗时间id = shi.清洗时间id
where bd.BUGUID = '248B1E17-AACB-E511-80B8-E41F13C51836'
    and bd.BldType = '产品楼栋'
group by BUGUID, shi.清洗时间, bi.区域
...
select  
    ...
    1 as 组织架构类型,
    -- 本年开工相关指标
    sum(case when DATEDIFF(year,shi.实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end) as 本年计划开工面积,
    sum(case when DATEDIFF(year,shi.实际开工实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end) as 本年实际开工面积,
    case 
        when sum(case when DATEDIFF(year,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(year,实际开工实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end) /
             sum(case when DATEDIFF(year,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end)
    end as 本年开工完成率,
    ...
    case 
        when sum(case when DATEDIFF(month,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(month,实际开工实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end) /
             sum(case when DATEDIFF(month,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end)
    end as 本月开工完成率,
    ...
    case 
        when sum(case when DATEDIFF(year,竣工备案计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, 清洗时间), 0))) = 0 then BuildArea end) = 0 then 0
...
select  
    ...
    2 as 组织架构类型,
    -- 本年开工相关指标
    sum(case when DATEDIFF(year,shi.实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本年计划开工面积,
    sum(case when DATEDIFF(year,shi.实际开工实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本年实际开工面积,
    case 
        when sum(case when DATEDIFF(year,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(year,实际开工实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) /
             sum(case when DATEDIFF(year,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end)
    end as 本年开工完成率,

    -- 本月开工相关指标
    sum(case when DATEDIFF(month,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本月计划开工面积,
    sum(case when DATEDIFF(month,实际开工实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本月实际开工面积,
    case 
        when sum(case when DATEDIFF(month,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(month,实际开工实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) /
             sum(case when DATEDIFF(month,实际开工计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end)
    end as 本月开工完成率,

    -- 本年竣工相关指标
    sum(case when DATEDIFF(year,shi.竣工备案计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本年计划竣工面积,
    sum(case when DATEDIFF(year,竣工备案实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本年实际竣工面积,
    case 
        when sum(case when DATEDIFF(year,竣工备案计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(year,竣工备案实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) /
             sum(case when DATEDIFF(year,竣工备案计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end)
    end as 本年竣工完成率,

    -- 本月竣工相关指标
    sum(case when DATEDIFF(month,竣工备案计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本月计划竣工面积,
    sum(case when DATEDIFF(month,竣工备案实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) as 本月实际竣工面积,
    case 
        when sum(case when DATEDIFF(year,竣工备案计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) = 0 then 0
        else sum(case when DATEDIFF(month,竣工备案实际完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end) /
             sum(case when DATEDIFF(month,竣工备案计划完成时间,DATEADD(ms, -3, DATEADD(mm, DATEDIFF(m, 0, shi.清洗时间), 0))) = 0 then BuildArea end)
    end as 本月竣工完成率
from data_wide_dws_mdm_Building bd
inner join dw_s_WqBaseStatic_ScheduleInfo shi on 组织架构类型 = 7 and bd.BuildingGUID = shi.组织架构ID
inner join dw_s_WqBaseStatic_BaseInfo bi on bi.组织架构id = shi.组织架构id and bi.清洗时间id = shi.清洗时间id
...
INTO    #RgQy_last
FROM    highdata_prod.dbo.dw_s_WqBaseStatic_Organization org
        LEFT JOIN highdata_prod.dbo.dw_s_WqBaseStatic_tradeInfo sale ON sale.组织架构id = org.组织架构id and  sale.组织架构类型 =org.组织架构类型--and org.清洗时间id = sale.清洗时间id
WHERE   1 = 1 AND   org.组织架构类型 in (1,2) AND  org.平台公司名称 = '湾区公司' 
group by 
        org.清洗时间,
        org.平台公司GUID ,
...
INTO    #hl_last
FROM    highdata_prod.dbo.dw_s_WqBaseStatic_Organization org
        LEFT JOIN highdata_prod.dbo.dw_s_WqBaseStatic_returnInfo hl ON hl.组织架构id = org.组织架构id and  hl.组织架构类型 =org.组织架构类型 --and org.清洗时间id = hl.清洗时间id
WHERE   1 = 1 AND   org.组织架构类型 in (1,2) AND  org.平台公司名称 = '湾区公司' 
group by 
        org.清洗时间,
        org.平台公司GUID ,
...
INTO    #xjl_last
FROM    highdata_prod.dbo.dw_s_WqBaseStatic_Organization org
        LEFT JOIN highdata_prod.dbo.dw_s_WqBaseStatic_cashflowInfo cash ON cash.组织架构id = org.组织架构id and  cash.组织架构类型 =org.组织架构类型  --and org.清洗时间id = cash.清洗时间id
WHERE   1 = 1 AND   org.组织架构类型 in (1,2) AND  org.平台公司名称 = '湾区公司' 
group by 
        org.清洗时间,
        org.平台公司GUID ,
...
left join #RgQy_last l on t.清洗时间 = l.清洗时间 and t.组织架构类型 = l.组织架构类型 and  t.组织架构名称 = l.组织架构名称
UNION ALL
--签约
SELECT  
...
FROM    #SubCompayMonthCshflow t
left join #hl_last l on t.清洗时间 = l.清洗时间 and t.组织架构类型 = l.组织架构类型 and  t.组织架构名称 = l.组织架构名称
UNION ALL
--回笼权益口径
SELECT  
        convert(datetime, t.清洗时间) as  清洗时间 ,
...
FROM    #SubCompayMonthCshflow t
left join #xjl_last l on t.清洗时间 = l.清洗时间 and t.组织架构类型 = l.组织架构类型 and  t.组织架构名称 = l.组织架构名称
UNION ALL
--直投
SELECT  
        convert(datetime, t.清洗时间) as  清洗时间 ,
...
FROM    #sh t
left join #Lastsh l on t.BUGUID=l.BUGUID and  t.清洗时间 = l.清洗时间 and t.组织架构类型 = l.组织架构类型 and  t.组织架构名称 = l.组织架构名称
UNION ALL
--竣工
SELECT  
        convert(datetime, t.清洗时间) as  清洗时间 ,
