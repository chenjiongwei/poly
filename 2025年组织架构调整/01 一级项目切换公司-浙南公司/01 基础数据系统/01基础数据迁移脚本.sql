...
-- AS
    BEGIN --获取待迁移信息表
        SELECT  * INTO  #dqy_proj FROM  dqy_proj_20250421;

        --修改项目所属公司
        IF OBJECT_ID(N'md_Project_bak_20250421', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.md_Project_bak_20250421
        ...
        IF OBJECT_ID(N'md_Project_work_bak_20250421', N'U') IS NULL
            SELECT  A.*
            INTO    dbo.md_Project_work_bak_20250421
        ...
        IF OBJECT_ID(N'md_Project2OrgCompany_bak_20250421', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.md_Project2OrgCompany_bak_20250421
        ...
        --房间信息表
        IF OBJECT_ID(N'md_Room_bak_20250421', N'U') IS NULL
            SELECT  A.*
            INTO    dbo.md_Room_bak_20250421
            FROM    dbo.md_Room A
                    INNER JOIN #dqy_proj B ON A.ProjGUID = B.OldProjGuid;
        ...
        --房间腾挪日志表  
        IF OBJECT_ID(N'md_RoomTengNuoLog_bak_20250421', N'U') IS NULL
            SELECT  A.*
            INTO    dbo.md_RoomTengNuoLog_bak_20250421
        ...
        --房间腾挪审核表
        IF OBJECT_ID(N'md_RoomTengNuoApprove_bak_20250421', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.md_RoomTengNuoApprove_bak_20250421
        ...
        --房间使用属性调整方案表
        IF OBJECT_ID(N'md_RoomPropertyAdjust_bak_20250421', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.md_RoomPropertyAdjust_bak_20250421
        ...
        -- md_ProjectOperLog 
        IF OBJECT_ID(N'md_ProjectOperLog_bak_20250421', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.md_ProjectOperLog_bak_20250421
        ...
        /*--增加组织架构公司跟平台公司的映射关系
        IF OBJECT_ID(N'companyjoin_bak_20250421', N'U') IS NULL
            SELECT  a.* INTO    dbo.companyjoin_bak_20250421 FROM   dbo.CompanyJoin a;
