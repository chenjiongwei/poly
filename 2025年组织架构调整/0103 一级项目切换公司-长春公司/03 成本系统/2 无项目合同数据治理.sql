...
WHERE   bu.buname = '长春公司'
        AND ISNULL(ProjectCodeList, '') = '';

--需要将公司下所有合同合并到新公司
SELECT DISTINCT  NewBuguid,NewBuname,OldBuguid,OldBuname
INTO #bu
FROM dqy_proj_20250422
  
--处理合同数据 
IF OBJECT_ID(N'cb_contract_bak_20250422_无项目合同', N'U') IS NULL
    SELECT cb.*
    INTO cb_contract_bak_20250422_无项目合同
    FROM cb_Contract cb
...
--处理预呈批数据 
IF OBJECT_ID(N'cb_Contract_Pre_bak_20250422_无项目合同', N'U') IS NULL
    SELECT cb.*
    INTO cb_Contract_Pre_bak_20250422_无项目合同
...
--处理合同类别
IF OBJECT_ID(N'cb_Contract2HTType_bak_20250422_无项目合同', N'U') IS NULL
    SELECT a.*
    INTO dbo.cb_Contract2HTType_bak_20250422_无项目合同
    ...
    FROM cb_Contract2HTType a
        INNER JOIN cb_Contract2HTType_bak_20250422_无项目合同 b
...
WHERE a.ContractGUID IN (   SELECT cb.ContractGUID
                            FROM cb_contract_bak_20250422_无项目合同 cb 
                        );
...
--刷新票据信息
IF OBJECT_ID(N'cb_InvoiceItem_bak_20250422_无项目合同', N'U') IS NULL
    SELECT a.*
    INTO dbo.cb_InvoiceItem_bak_20250422_无项目合同
...
--刷新合同变更
IF OBJECT_ID(N'cb_HTAlter_bak_20250422_无项目合同', N'U') IS NULL
    SELECT a.*
    INTO cb_HTAlter_bak_20250422_无项目合同
    FROM cb_HTAlter a
...
--刷新合同付款申请
IF OBJECT_ID(N'cb_HTFKApply_bak_20250422_无项目合同', N'U') IS NULL
    SELECT a.*
    INTO cb_HTFKApply_bak_20250422_无项目合同
    FROM cb_HTFKApply a
        LEFT JOIN dbo.cb_Contract b
            ON a.ContractGUID = b.ContractGUID
    WHERE a.BUGUID <> b.BUGUID
...
--刷新合同付款计划
IF OBJECT_ID(N'cb_HTFKPlan_bak_20250422_无项目合同', N'U') IS NULL
    SELECT a.*
    INTO cb_HTFKPlan_bak_20250422_无项目合同
    FROM cb_HTFKPlan a
        inner JOIN dbo.cb_Contract b
            ON a.ContractGUID = b.ContractGUID
    WHERE a.BUGUID <> b.BUGUID
...
--刷新应收明细
IF OBJECT_ID(N'cb_payfeebill_bak_20250422_无项目合同', N'U') IS NULL
    SELECT bil.*
    INTO cb_payfeebill_bak_20250422_无项目合同
