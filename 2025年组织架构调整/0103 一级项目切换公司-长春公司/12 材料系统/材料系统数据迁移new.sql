...
BEGIN
    ...
    SELECT  p.HierarchyCode ,
            ...
            t.*
    INTO    #dqy_proj
    FROM    -- [172.16.4.129].MyCost_Erp352.dbo.dqy_proj_20250422 t
            MyCost_Erp352.dbo.dqy_proj_20250422 t
	...
	*/
    ...
    IF OBJECT_ID(N'cl_ClCompany2ServiceProject_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    cl_ClCompany2ServiceProject_bak_20250422
    ...
    IF OBJECT_ID(N'cl_ServiceProject_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    cl_ServiceProject_bak_20250422
    ...
    --处理业务数据
    --材料申请表
    IF OBJECT_ID(N'cl_Apply_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    cl_Apply_bak_20250422
        FROM    cl_Apply a
    ...
    --订单表
    IF OBJECT_ID(N'cl_Order_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    cl_Order_bak_20250422
        FROM    cl_Order a
                INNER JOIN dbo.p_Project p ON a.ProjGUID = p.p_projectId
        WHERE   a.ProjGUID IN(SELECT    OldProjGuid FROM    #dqy_proj);
    ...
    --验收单
    IF OBJECT_ID(N'cl_Recipient_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    cl_Recipient_bak_20250422
        FROM    cl_Recipient a
                INNER JOIN dbo.p_Project p ON a.ProjGUID = p.p_projectId
        WHERE   a.ProjGUID IN(SELECT    OldProjGuid FROM    #dqy_proj);
    ...
    --退货单
    IF OBJECT_ID(N'x_cl_ReturnOrder_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    x_cl_ReturnOrder_bak_20250422
    ...
    --材料合同表
    IF OBJECT_ID(N'cl_Contract_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    cl_Contract_bak_20250422
        FROM    cl_Contract a
                INNER JOIN dbo.p_Project p ON a.ProjGUID = p.p_projectId
        WHERE   a.ProjGUID IN(SELECT    OldProjGuid FROM    #dqy_proj);
    ...
    --实际用料计划表
    IF OBJECT_ID(N'cl_ProductRequirement_bak_20250422', N'U') IS NULL
        SELECT  a.*
        INTO    cl_ProductRequirement_bak_20250422
