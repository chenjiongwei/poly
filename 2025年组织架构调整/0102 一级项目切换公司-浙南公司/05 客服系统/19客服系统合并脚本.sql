...
--备份
SELECT  * INTO  myBizParamOption_bak20250421 FROM dbo.myBizParamOption;
...
BEGIN
    ...
    SELECT *
    INTO #dqy_proj
    FROM dqy_proj_20250421 t;

    -- --受理单位表
    PRINT '受理单位表k_AcceptBU';

    IF OBJECT_ID(N'k_AcceptBU_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_AcceptBU_bak20250421
        FROM k_AcceptBU a
    ...
    IF OBJECT_ID(N'k_Task_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_Task_bak20250421
        FROM k_Task a
            INNER JOIN #dqy_proj p
    ...
    IF OBJECT_ID(N'k_Receive_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_Receive_bak20250421
        FROM k_Receive a
            INNER JOIN #dqy_proj p
                ON p.OldProjGuid = a.ProjGUID
        WHERE a.BUGUID <> p.NewBuguid;
    ...
    IF OBJECT_ID(N'k_Complaint_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_Complaint_bak20250421
        FROM k_Complaint a
            INNER JOIN #dqy_proj p
                ON p.OldProjGuid = a.ProjGUID
        WHERE a.BUGUID <> p.NewBuguid;
    ...
    --赔付记录实体k_Pay --没有数据
    IF OBJECT_ID(N'k_Pay_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_Pay_bak20250421
        FROM k_Pay a
            INNER JOIN #dqy_proj p
                ON p.OldProjGuid = a.ProjGUID
        WHERE a.BUGUID <> p.NewBuguid;
    ...
    -- /////////////////////////////// 2025年组织架构调整新增处理表 开始 /////////////////////////////////////////////
    IF OBJECT_ID(N'k_CooperativeProjectDelivery_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_CooperativeProjectDelivery_bak20250421
    ...
    IF OBJECT_ID(N'k_GreatComplaint_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_GreatComplaint_bak20250421
    ...
    IF OBJECT_ID(N'k_HZXMJFLR_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_HZXMJFLR_bak20250421
        FROM k_HZXMJFLR a
            INNER JOIN #dqy_proj p
                ON p.OldProjGuid = a.ProjGUID
        WHERE a.BUGUID <> p.NewBuguid;
    ...
    IF OBJECT_ID(N'k_Receivedellog_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_Receivedellog_bak20250421
    ...
    IF OBJECT_ID(N'k_taskdellog_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO k_taskdellog_bak20250421
        FROM k_taskdellog a
            INNER JOIN #dqy_proj p
                ON p.OldProjGuid = a.ProjGUID
        WHERE a.BUGUID <> p.NewBuguid;
    ...
    PRINT 'k_taskdellog' + CONVERT(NVARCHAR(20), @@ROWCOUNT);
        ...
        -- 年交付计划表
    IF OBJECT_ID(N's_YearJLPlan_bak20250421', N'U') IS NULL
        SELECT a.*
        INTO s_YearJLPlan_bak20250421
        FROM s_YearJLPlan a
