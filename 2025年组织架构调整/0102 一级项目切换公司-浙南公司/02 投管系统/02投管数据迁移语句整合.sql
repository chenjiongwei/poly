...
-- AS
    ...
    BEGIN
        ...
        SELECT  * INTO  #t FROM dqy_proj_20250421;

        PRINT '=====开始25迁移=====';

        IF OBJECT_ID(N'mdm_Project_bak_20250421', N'U') IS NULL
            SELECT  mp.*
            INTO    mdm_Project_bak_20250421
            FROM    mdm_Project mp
        ...
        IF OBJECT_ID(N'mdm_Project_Sync_bak_20250421', N'U') IS NULL
            SELECT  mp.*
            INTO    mdm_Project_Sync_bak_20250421
        ...
        --备份表
        IF OBJECT_ID(N'mdm_BldAddType_bak_20250421', N'U') IS NULL
            SELECT  mp.*
            INTO    mdm_BldAddType_bak_20250421
        ...
        IF OBJECT_ID(N'mdm_GCBuildIndexSet_bak_20250421', N'U') IS NULL
            SELECT  mp.*
            INTO    mdm_GCBuildIndexSet_bak_20250421
        ...
        IF OBJECT_ID(N'mdm_ProjCheckLevel_bak_20250421', N'U') IS NULL
            SELECT  mp.*
            INTO    mdm_ProjCheckLevel_bak_20250421
        ...
        IF OBJECT_ID(N'p_LandInfo_bak_20250421', N'U') IS NULL
            SELECT  ld.*
            INTO    p_LandInfo_bak_20250421
        ...
        /*	--刷新城市参数
			...
			WHERE   op.ParamName = 'td_city' AND EXISTS (SELECT 1
															FROM   dbo.myBizParamOption a
																...
																INNER JOIN dqy_proj_20250421 t ON t.OldProjGUID = mp.ProjGUID OR mp.ParentProjGUID = t.OldProjGUID
        ...
        --刷新项目公司数据
        --备份
        IF OBJECT_ID(N'p_DevelopmentCompany_xm_bak_20250421', N'U') IS NULL
            SELECT  pdc.*
            INTO    p_DevelopmentCompany_xm_bak_20250421
        ...
        -- // 未关联项目的项目公司信息单独刷新
        -- 备份
        IF OBJECT_ID(N'p_DevelopmentCompany_bak_20250421', N'U') IS NULL
        BEGIN
            SELECT  dc.* 
            INTO    p_DevelopmentCompany_bak_20250421  
        ...
        -- // 刷新项目公司的财务管理公司信息
        -- 备份
        IF OBJECT_ID(N'p_DevelopmentCompany_FinancialInfo_bak_20250421', N'U') IS NULL
            SELECT  fin.* 
            INTO    p_DevelopmentCompany_FinancialInfo_bak_20250421 
        ...
        --缓存对应合作方公司数据
        -- 备份p_BelongCompany
        IF OBJECT_ID(N'p_BelongCompany_bak_20250421', N'U') IS NULL
            SELECT  a.* INTO    p_BelongCompany_bak_20250421 FROM   p_BelongCompany a;
