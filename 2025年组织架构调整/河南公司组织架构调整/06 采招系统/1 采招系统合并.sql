...
-- AS
    BEGIN
        ...
        --开始刷新项目所属公司GUID和项目编码
        IF OBJECT_ID(N'cg_DocArchive_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_DocArchive_bak_20250411
        ...
        IF OBJECT_ID(N'cg_CgApply_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_CgApply_bak_20250411
            FROM    cg_CgApply a
                    INNER JOIN #proj p ON a.ProjGUID = p.ProjGUID
            WHERE   a.ProjCode <> p.ProjCode;
        ...
        IF OBJECT_ID(N'cg_CgPlan_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_CgPlan_bak_20250411
            FROM    cg_CgPlan a
        ...
        IF OBJECT_ID(N'cg_CgSolution_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_CgSolution_bak_20250411
        ...
        IF OBJECT_ID(N'cg_CgSolutionLinkedPlan_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_CgSolutionLinkedPlan_bak_20250411
        ...
        IF OBJECT_ID(N'cg_PG2Contract_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_PG2Contract_bak_20250411
        ...
        IF OBJECT_ID(N'cg_PGPlan2Contract_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_PGPlan2Contract_bak_20250411
        ...
        IF OBJECT_ID(N'cg_PGPlan_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_PGPlan_bak_20250411
            FROM    cg_PGPlan a
                    INNER JOIN #proj p ON CHARINDEX(CONVERT(VARCHAR(50), p.ProjGUID), a.ProjGUIDList) <> 0
        ...
        IF OBJECT_ID(N'Cg_CgPlanSp_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.Cg_CgPlanSp_bak_20250411
            FROM    Cg_CgPlanSp a
        ...
        IF OBJECT_ID(N'Cg_CgPlan_OriginSp_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.Cg_CgPlan_OriginSp_bak_20250411
        ...
        IF OBJECT_ID(N'cg_cgplan_origin_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_cgplan_origin_bak_20250411
        ...
        IF OBJECT_ID(N'cg_PGProjReceipt_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_PGProjReceipt_bak_20250411
        ...
        IF OBJECT_ID(N'cg_Contract2CgProc_bak_20250411', N'U') IS NULL
            SELECT  a.*
            INTO    dbo.cg_Contract2CgProc_bak_20250411
     ...
     */
        IF OBJECT_ID(N'myWorkflowProcessEntity_bak_20250411', N'U') IS NULL
            SELECT  w.*
            INTO    dbo.myWorkflowProcessEntity_bak_20250411
            FROM    cg_cgsolution a
                    INNER JOIN dbo.myWorkflowProcessEntity w ON w.BusinessGUID = a.CgSolutionGUID
                    INNER JOIN #proj p ON CHARINDEX(CONVERT(VARCHAR(50), p.ProjGUID), a.ProjGUIDList) <> 0
            WHERE   a.BUGUID <> w.BUGUID AND w.ProcessStatus = 2    --AND a.BUGUID IN(SELECT buguid FROM #proj)
