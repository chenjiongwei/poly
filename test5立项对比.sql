--获取项目首推时间
select t.projguid, min(SJkpxsDate) as SJkpxsDate 
into #stDate
from (
--获取房间认购的时间
select parentprojguid projguid, min(qsdate) as SJkpxsDate 
from data_wide_s_SaleHsData group by parentprojguid
--having sum(isnull(RoomTotal,0))<>0
--获取特殊业绩计算货量的录入时间
union all 
select ParentProjGUID,min(StatisticalDate) as SJkpxsDate
from data_wide_s_SpecialPerformance
where TsyjType in (select TsyjTypeName from [172.16.4.141].erp25.dbo.s_TsyjType t where IsCalcYSHL =1)
