...
* 功能描述：生成存货分析报表下新样式，按并表/不并表、产成品/在建开发成本/停工缓建/土地储备等分类统计
* 参数说明：
*   @var_buguid  公司GUID，多个用逗号分隔
* 使用示例：
*   exec [usp_cb_存货分析报表]  '275B7A4E-4AB3-408E-A43A-1BA6AC5B0321' -- 广东公司
* 测试项目：
   ...
   --- 股东投入余额
    ...
    FROM (
        ...
        SELECT 
            ...
            sum(case  when  isAllBuildKg = 1 and ( F056ld.赛道图标签 in ('D1-已开业未融资','D2-已开业已融资','D3-未开业') 
            or  F056ld.是否持有='是' or   isnull(F056ld.持有面积,0) >0 ) and isnull(IsCost,0) =0 then isnull(Zjm,0) else 0 end ) as 已开工分期_已开工分期未售存货不存成本面积,
            -- 存货利润分析  
