...
-- 公司资源宽表 data_wide_dws_s_WqBaseStatic_CompanyResource
BEGIN
    -- =============================================
    -- 第一部分：基础数据准备
    -- =============================================
    ...
    */
    ...
    SELECT dv.DevelopmentCompanyGUID AS OrgGUID,
           ...
           ISNULL(
               CASE
                   ...
                   THEN
                       ISNULL(zs.持有面积, 0) * ISNULL(hz.本年签约均价, 0)

                   WHEN ISNULL(hz.业态均价, 0) <> 0
                        AND ms.ProductType NOT IN ('公建配套', '后勤区', '会所', '会展', '剧院', '其他', '学校', '养老', '酒店', '体育馆')
                        AND ms.ProductName NOT IN ('人防车位', '人防车库')
                        AND ms.BusinessType NOT IN ('还建房', '回迁房', '廉租房')
                   THEN
                       ISNULL(zs.持有面积, 0) * ISNULL(hz.业态均价, 0)
                   ...
                   THEN
                       CASE
                           WHEN ms.ProductType = '地下室/车库' THEN
                               ISNULL(zs.持有套数, 0) * ISNULL(hz.预测单价, 0) / 10000.0
                           ELSE
                               ISNULL(zs.持有面积, 0) * ISNULL(hz.预测单价, 0)
                       END
               END,
               0
           ) AS 反算总货值_总货值, -- 动态总可售货值+持有货值

           ISNULL(hz.总可售面积 / 10000.0, 0) +
           CASE
               WHEN
                   ms.ProductType NOT IN ('公建配套', '后勤区', '会所', '会展', '剧院', '其他', '学校', '养老', '酒店', '体育馆')
                   AND ms.ProductName NOT IN ('人防车位', '人防车库')
                   AND ms.BusinessType NOT IN ('还建房', '回迁房', '廉租房')
               THEN ISNULL(zs.持有面积, 0)
               ELSE 0
           END AS 反算总货值_总面积, --动态总可售面积+持有面积
           ...
           CASE
               WHEN
                   ...
                   AND ms.ProductName NOT IN ('人防车位', '人防车库')
                   AND ms.BusinessType NOT IN ('还建房', '回迁房', '廉租房')
               THEN ISNULL(zs.持有套数, 0)
               ELSE 0
           END AS 反算总货值_套数, --动态总可售套数+持有套数           

           ISNULL(hz.待售货值 / 10000, 0) +
           ISNULL(
               CASE
                   WHEN ISNULL(hz.本年签约均价, 0) <> 0
                        AND ms.ProductType NOT IN ('公建配套', '后勤区', '会所', '会展', '剧院', '其他', '学校', '养老', '酒店', '体育馆')
    ...
    SELECT  *,
            ...
            case when 实际开工完成日期 IS NOT NULL THEN 反算总货值_套数 ELSE 0 end as 已开工情况_已开工套数,
            ...
            CASE WHEN 实际开工完成日期 IS NOT NULL THEN 已售套数 ELSE 0 END AS 已开工情况_已开工已售套数,
            ...
            CASE WHEN 实际开工完成日期 IS NOT NULL THEN 反算总货值_剩余套数 ELSE 0 END AS 已开工情况_已开工未售套数, -- 已开工的反算剩余面积
            -- 存货情况
            -- 计算已达到预售形象的存货情况
            ...
            CASE WHEN 实际开工完成日期 IS NOT NULL AND 达到预售形象完成日期 IS NOT NULL THEN 反算总货值_剩余套数 ELSE 0 END AS 存货情况_存货套数, -- 达预售形象的反算剩余面积
            -- 自持情况
            -- 计算自持资产的货值和面积
            ...
            CASE WHEN 赛道图标签 IN ('D1-已开业未融资','D2-已开业已融资','D3-未开业') THEN 反算总货值_剩余套数 ELSE 0 END 
              + CASE WHEN 赛道图标签 NOT IN ('D1-已开业未融资','D2-已开业已融资','D3-未开业') THEN 持有套数 ELSE 0 END AS 自持情况_总自持资产套数, -- 自持资产总套数
            ...
            CASE WHEN 赛道图标签 IN ('D1-已开业未融资','D2-已开业已融资','D3-未开业') THEN 反算总货值_剩余套数 ELSE 0 END AS 自持情况_已转经营套数,
            ...
            CASE WHEN 实际开工完成日期 IS NOT NULL AND 达到预售形象完成日期 IS NULL THEN 反算总货值_剩余套数 ELSE 0 END AS 在途情况_在途套数, 
            ...
            CASE WHEN 实际开工完成日期 IS NULL THEN 反算总货值_剩余套数 ELSE 0 END AS 未开工情况_未开工套数, -- 尚未开工的反算剩余面积   
            ...
            CASE WHEN 处置后去向 = '分货' THEN 反算总货值_剩余套数 * 股权比例 / 100.0 ELSE 0 END AS 分货转经营套数, -- 处置后去向标签为分货的反算剩余货值*我司股比
            ...
            CASE WHEN 处置后去向 = '分货' THEN 反算总货值_剩余套数 * (1 - 股权比例 / 100.0) ELSE 0 END AS 分货销售套数,

            -- 处置前 
            case when 处置前五类 in ('合作受阻','开发受限','停工缓建','投资未落实') then 反算总货值_剩余面积 else 0  end as 处置前临时冻结面积合计,
            case when 处置前五类 ='合作受阻' then 反算总货值_剩余面积 else 0  end as 处置前临时冻结_合作受阻冻结面积,
            case when 处置前五类 ='开发受限' then 反算总货值_剩余面积 else 0  end as 处置前临时冻结_开发受限冻结面积,
            case when 处置前五类 ='停工缓建' then 反算总货值_剩余面积 else 0  end as 处置前临时冻结_停工缓建冻结面积,
            case when 处置前五类 ='投资未落实' then 反算总货值_剩余面积 else 0  end as 处置前临时冻结_投资未落实冻结面积,

            case when 处置前五类 in ('合作受阻','开发受限','停工缓建','投资未落实') then 反算总货值_剩余货值 else 0  end as 处置前临时冻结货值合计,
            case when 处置前五类 ='合作受阻' then 反算总货值_剩余货值 else 0  end as 处置前临时冻结_合作受阻冻结货值,
            case when 处置前五类 ='开发受限' then 反算总货值_剩余货值 else 0  end as 处置前临时冻结_开发受限冻结货值,
            case when 处置前五类 ='停工缓建' then 反算总货值_剩余货值 else 0  end as 处置前临时冻结_停工缓建冻结货值,
            case when 处置前五类 ='投资未落实' then 反算总货值_剩余货值 else 0  end as 处置前临时冻结_投资未落实冻结货值,

            case when 处置前五类 in ('合作受阻','开发受限','停工缓建','投资未落实') then 反算总货值_剩余套数 else 0  end as 处置前临时冻结套数合计,
            case when 处置前五类 ='合作受阻' then 反算总货值_剩余套数 else 0  end as 处置前临时冻结_合作受阻冻结套数,
            case when 处置前五类 ='开发受限' then 反算总货值_剩余套数 else 0  end as 处置前临时冻结_开发受限冻结套数,
            case when 处置前五类 ='停工缓建' then 反算总货值_剩余套数 else 0  end as 处置前临时冻结_停工缓建冻结套数,
            case when 处置前五类 ='投资未落实' then 反算总货值_剩余套数 else 0  end as 处置前临时冻结_投资未落实冻结套数,


            case when 处置前五类 ='退换调转' then 反算总货值_剩余面积 else 0  end as 处置前退换调转剩余面积,
            case when 处置前五类 ='退换调转' then 反算总货值_剩余货值 else 0  end as 处置前退换调转剩余货值,
            case when 处置前五类 ='退换调转' then 反算总货值_剩余套数 else 0  end as 处置前退换调转剩余套数,

            case when 处置前五类 ='正常销售' then 反算总货值_剩余面积 else 0  end as 处置前正常销售剩余面积,
            case when 处置前五类 ='正常销售' then 反算总货值_剩余货值 else 0  end as 处置前正常销售剩余货值,
            case when 处置前五类 ='正常销售' then 反算总货值_剩余套数 else 0  end as 处置前正常销售剩余套数,

            case when 处置前五类 ='转经营' then 反算总货值_剩余面积 else 0  end as 处置前转经营剩余面积,
            case when 处置前五类 ='转经营' then 反算总货值_剩余货值 else 0  end as 处置前转经营剩余货值,
            case when 处置前五类 ='转经营' then 反算总货值_剩余套数 else 0  end as 处置前转经营剩余套数,

            case when 处置前五类 ='尾盘' then 反算总货值_剩余面积 else 0  end as 处置前尾盘剩余面积,
            case when 处置前五类 ='尾盘' then 反算总货值_剩余货值 else 0  end as 处置前尾盘剩余货值,
            case when 处置前五类 ='尾盘' then 反算总货值_剩余套数 else 0  end as 处置前尾盘剩余套数,

            -- 处置后
            case when 处置后去向 in ('合作受阻','开发受限','停工缓建','投资未落实') then 反算总货值_剩余面积 else 0  end as 处置后临时冻结面积合计,
            case when 处置后去向 ='合作受阻' then 反算总货值_剩余面积 else 0  end as 处置后临时冻结_合作受阻冻结面积,
            case when 处置后去向 ='开发受限' then 反算总货值_剩余面积 else 0  end as 处置后临时冻结_开发受限冻结面积,
            case when 处置后去向 ='停工缓建' then 反算总货值_剩余面积 else 0  end as 处置后临时冻结_停工缓建冻结面积,
            case when 处置后去向 ='投资未落实' then 反算总货值_剩余面积 else 0  end as 处置后临时冻结_投资未落实冻结面积,

            case when 处置后去向 in ('合作受阻','开发受限','停工缓建','投资未落实') then 反算总货值_剩余货值 else 0  end as 处置后临时冻结货值合计,
            case when 处置后去向 ='合作受阻' then 反算总货值_剩余货值 else 0  end as 处置后临时冻结_合作受阻冻结货值,
            case when 处置后去向 ='开发受限' then 反算总货值_剩余货值 else 0  end as 处置后临时冻结_开发受限冻结货值,
            case when 处置后去向 ='停工缓建' then 反算总货值_剩余货值 else 0  end as 处置后临时冻结_停工缓建冻结货值,
            case when 处置后去向 ='投资未落实' then 反算总货值_剩余货值 else 0  end as 处置后临时冻结_投资未落实冻结货值,

            case when 处置后去向 in ('合作受阻','开发受限','停工缓建','投资未落实') then 反算总货值_剩余套数 else 0  end as 处置后临时冻结套数合计,
            case when 处置后去向 ='合作受阻' then 反算总货值_剩余套数 else 0  end as 处置后临时冻结_合作受阻冻结套数,
            case when 处置后去向 ='开发受限' then 反算总货值_剩余套数 else 0  end as 处置后临时冻结_开发受限冻结套数,
            case when 处置后去向 ='停工缓建' then 反算总货值_剩余套数 else 0  end as 处置后临时冻结_停工缓建冻结套数,
            case when 处置后去向 ='投资未落实' then 反算总货值_剩余套数 else 0  end as 处置后临时冻结_投资未落实冻结套数,


            case when 处置后去向 ='退换调转' then 反算总货值_剩余面积 else 0  end as 处置后退换调转剩余面积,
            case when 处置后去向 ='退换调转' then 反算总货值_剩余货值 else 0  end as 处置后退换调转剩余货值,
            case when 处置后去向 ='退换调转' then 反算总货值_剩余套数 else 0  end as 处置后退换调转剩余套数,

            case when 处置后去向 ='正常销售' then 反算总货值_剩余面积 else 0  end as 处置后正常销售剩余面积,
            case when 处置后去向 ='正常销售' then 反算总货值_剩余货值 else 0  end as 处置后正常销售剩余货值,
            case when 处置后去向 ='正常销售' then 反算总货值_剩余套数 else 0  end as 处置后正常销售剩余套数,

            case when 处置后去向 ='转经营' then 反算总货值_剩余面积 else 0  end as 处置后转经营剩余面积,
            case when 处置后去向 ='转经营' then 反算总货值_剩余货值 else 0  end as 处置后转经营剩余货值,
            case when 处置后去向 ='转经营' then 反算总货值_剩余套数 else 0  end as 处置后转经营剩余套数,

            case when 处置后去向 ='尾盘' then 反算总货值_剩余面积 else 0  end as 处置后尾盘剩余面积,
            case when 处置后去向 ='尾盘' then 反算总货值_剩余货值 else 0  end as 处置后尾盘剩余货值,
            case when 处置后去向 ='尾盘' then 反算总货值_剩余套数 else 0  end as 处置后尾盘剩余套数,
            

            case when 是否开工 ='是' and  是否停工 ='否' and 推售状态 ='是' then 反算总货值_剩余面积 else 0 end  as 已开工剩余货量_已推未售剩余面积, --已推未售的已开工反算剩余资源
            case when 是否开工 ='是' and  是否停工 ='否' and 推售状态 ='是' then 反算总货值_剩余货值  else 0 end  as 已开工剩余货量_已推未售剩余货值,
            case when 是否开工 ='是' and  是否停工 ='否' and 推售状态 ='是' then 反算总货值_剩余套数 else 0 end  as 已开工剩余货量_已推未售剩余套数,

            case when 是否开工 ='是' and  是否停工 ='否' and 推售状态 ='否' and  预售办理完成日期  is not null  then 反算总货值_剩余面积 else 0 end  已开工剩余货量_获证待推剩余面积, -- 获证待推的已开工反算剩余资源
            case when 是否开工 ='是' and  是否停工 ='否' and 推售状态 ='否' and  预售办理完成日期  is not null  then 反算总货值_剩余货值 else 0 end 已开工剩余货量_获证待推剩余货值,
            case when 是否开工 ='是' and  是否停工 ='否' and 推售状态 ='否' and  预售办理完成日期  is not null  then 反算总货值_剩余套数 else 0 end  已开工剩余货量_获证待推剩余套数,

            case when 是否开工 ='是' and  是否停工 ='否' and  预售办理完成日期 is null and 达到预售形象完成日期 is not null  then  反算总货值_剩余面积 else  0  end 已开工剩余货量_达形象未取证剩余面积, --达形象未取证的已开工反算剩余资源
            case when 是否开工 ='是' and  是否停工 ='否' and  预售办理完成日期 is null and 达到预售形象完成日期 is not null  then  反算总货值_剩余货值 else  0  end 已开工剩余货量_达形象未取证剩余货值,
            case when 是否开工 ='是' and  是否停工 ='否' and  预售办理完成日期 is null and 达到预售形象完成日期 is not null  then  反算总货值_剩余套数 else  0  end 已开工剩余货量_达形象未取证剩余套数,

            case when 是否开工 ='是' and  是否停工 ='否' and  达到预售形象完成日期 IS NULL  then  反算总货值_剩余面积 else  0  end 已开工剩余货量_正常在途剩余面积,
            case when 是否开工 ='是' and  是否停工 ='否' and  达到预售形象完成日期 IS NULL  then  反算总货值_剩余货值 else  0  end 已开工剩余货量_正常在途剩余货值,
            case when 是否开工 ='是' and  是否停工 ='否' and  达到预售形象完成日期 IS NULL  then  反算总货值_剩余套数 else  0  end 已开工剩余货量_正常在途剩余套数,

            case when 是否开工 ='是' and  是否停工 ='是' and  达到预售形象完成日期 IS NULL  then  反算总货值_剩余面积 else  0  end 已开工剩余货量_停工缓建在途剩余面积,
            case when 是否开工 ='是' and  是否停工 ='是' and  达到预售形象完成日期 IS NULL  then  反算总货值_剩余货值 else  0  end 已开工剩余货量_停工缓建在途剩余货值,
            case when 是否开工 ='是' and  是否停工 ='是' and  达到预售形象完成日期 IS NULL  then  反算总货值_剩余套数 else  0  end 已开工剩余货量_停工缓建在途剩余套数,

            case when 是否开工 ='否' and  是否停工 ='是' then  反算总货值_剩余面积 else  0  end 未开工剩余货量_停工缓建剩余面积,
            case when 是否开工 ='否' and  是否停工 ='是' then  反算总货值_剩余货值 else  0  end 未开工剩余货量_停工缓建剩余货值,
            case when 是否开工 ='否' and  是否停工 ='是' then  反算总货值_剩余套数 else  0  end 未开工剩余货量_停工缓建剩余套数,

            -- 预计三年内开工：实际开工完成日期在未来三年内，且未开工、未停工
            case when 是否开工 ='否' and 是否停工 ='否'  and 实际开工计划日期 is not null  and datediff(year, getdate(), 实际开工计划日期) between 0 and 3 
                then 反算总货值_剩余面积 else 0 end 未开工剩余货量_预计三年内开工剩余面积,
            case when 是否开工 ='否' and 是否停工 ='否' and 实际开工计划日期 is not null  and datediff(year, getdate(), 实际开工计划日期) between 0 and 3 
                then 反算总货值_剩余货值 else 0 end 未开工剩余货量_预计三年内开工剩余货值,
            case when 是否开工 ='否' and 是否停工 ='否'  and 实际开工计划日期 is not null and datediff(year, getdate(), 实际开工计划日期) between 0 and 3 
                then 反算总货值_剩余套数 else 0 end 未开工剩余货量_预计三年内开工剩余套数,

            case when 是否开工 ='否' and  是否停工 ='否' and 实际开工计划日期 is not null  and datediff(year, getdate(), 实际开工计划日期) >3 then  反算总货值_剩余面积 else  0  end 未开工剩余货量_预计三年后开工剩余面积,
            case when 是否开工 ='否' and  是否停工 ='否' and 实际开工计划日期 is not null  and datediff(year, getdate(), 实际开工计划日期) >3 then  反算总货值_剩余货值 else  0  end 未开工剩余货量_预计三年后开工剩余货值,
            case when 是否开工 ='否' and  是否停工 ='否' and 实际开工计划日期 is not null  and datediff(year, getdate(), 实际开工计划日期) >3 then  反算总货值_剩余套数 else  0  end 未开工剩余货量_预计三年后开工剩余套数,

            -- 新增字段 建设状态
            case when 是否开工 ='是' and  竣备情况 ='是' then  反算总货值_剩余面积 else  0  end 建设状态_已竣备剩余面积,
            case when 是否开工 ='是' and  竣备情况 ='是' then  反算总货值_剩余货值 else  0  end 建设状态_已竣备剩余货值,
            case when 是否开工 ='是' and  竣备情况 ='是' then  反算总货值_剩余套数 else  0  end 建设状态_已竣备剩余套数,

            case when 是否开工 ='是' and  竣备情况 ='否' and 达到预售形象完成日期 is not null   then  反算总货值_剩余面积 else  0  end 建设状态_已达形象未达竣备剩余面积, --已达预售形象尚未竣备的反算剩余资源
            case when 是否开工 ='是' and  竣备情况 ='否' and 达到预售形象完成日期 is not null   then  反算总货值_剩余货值 else  0  end 建设状态_已达形象未达竣备剩余货值,
            case when 是否开工 ='是' and  竣备情况 ='否' and 达到预售形象完成日期 is not null   then  反算总货值_剩余套数 else  0  end 建设状态_已达形象未达竣备剩余套数,      

            case when 是否开工 ='是' and  达到预售形象完成日期 is null then  反算总货值_剩余面积 else  0  end 建设状态_已开工未达形象剩余面积,
            case when 是否开工 ='是' and  达到预售形象完成日期 is null then  反算总货值_剩余货值 else  0  end 建设状态_已开工未达形象剩余货值,
            case when 是否开工 ='是' and  达到预售形象完成日期 is null then  反算总货值_剩余套数 else  0  end 建设状态_已开工未达形象剩余套数,

            case when 是否停工 ='是' then  反算总货值_剩余面积 else  0  end 建设状态_停工缓建剩余面积,
            case when 是否停工 ='是' then  反算总货值_剩余货值 else  0  end 建设状态_停工缓建剩余货值,
            case when 是否停工 ='是' then  反算总货值_剩余套数 else  0  end 建设状态_停工缓建剩余套数       
    into  #result
    FROM #rest
    where  管理方式 <>'代建'
    ...
    SELECT
        ...
        -- 新增字段
        rst.处置前临时冻结面积合计,
        rst.处置前临时冻结_合作受阻冻结面积,
        rst.处置前临时冻结_开发受限冻结面积,
        rst.处置前临时冻结_停工缓建冻结面积,
        rst.处置前临时冻结_投资未落实冻结面积,

        rst.处置前临时冻结货值合计,
        rst.处置前临时冻结_合作受阻冻结货值,
        rst.处置前临时冻结_开发受限冻结货值,
        rst.处置前临时冻结_停工缓建冻结货值,
        rst.处置前临时冻结_投资未落实冻结货值,

        rst.处置前临时冻结套数合计,
        rst.处置前临时冻结_合作受阻冻结套数,
        rst.处置前临时冻结_开发受限冻结套数,
        rst.处置前临时冻结_停工缓建冻结套数,
        rst.处置前临时冻结_投资未落实冻结套数,
        ...
        -- 处置后
        rst.处置后临时冻结面积合计,
        rst.处置后临时冻结_合作受阻冻结面积,
        rst.处置后临时冻结_开发受限冻结面积,
        rst.处置后临时冻结_停工缓建冻结面积,
        rst.处置后临时冻结_投资未落实冻结面积,

        rst.处置后临时冻结货值合计,
        rst.处置后临时冻结_合作受阻冻结货值,
        rst.处置后临时冻结_开发受限冻结货值,
        rst.处置后临时冻结_停工缓建冻结货值,
        rst.处置后临时冻结_投资未落实冻结货值,

        rst.处置后临时冻结套数合计,
        rst.处置后临时冻结_合作受阻冻结套数,
        rst.处置后临时冻结_开发受限冻结套数,
        rst.处置后临时冻结_停工缓建冻结套数,
        rst.处置后临时冻结_投资未落实冻结套数,
        ...
        -- 新增字段
        rst.已开工剩余货量_已推未售剩余面积,
        rst.已开工剩余货量_已推未售剩余货值,
        rst.已开工剩余货量_已推未售剩余套数,

        rst.已开工剩余货量_获证待推剩余面积,
        rst.已开工剩余货量_获证待推剩余货值,
        rst.已开工剩余货量_获证待推剩余套数,
        
        rst.已开工剩余货量_达形象未取证剩余面积,
        rst.已开工剩余货量_达形象未取证剩余货值,
        rst.已开工剩余货量_达形象未取证剩余套数,
         
        rst.已开工剩余货量_正常在途剩余面积,
        rst.已开工剩余货量_正常在途剩余货值,
        rst.已开工剩余货量_正常在途剩余套数,

        rst.已开工剩余货量_停工缓建在途剩余面积,
        rst.已开工剩余货量_停工缓建在途剩余货值,
        rst.已开工剩余货量_停工缓建在途剩余套数,

        rst.未开工剩余货量_停工缓建剩余面积,
        rst.未开工剩余货量_停工缓建剩余货值,
        rst.未开工剩余货量_停工缓建剩余套数,

        rst.未开工剩余货量_预计三年内开工剩余面积,
        rst.未开工剩余货量_预计三年内开工剩余货值,
        rst.未开工剩余货量_预计三年内开工剩余套数,

        rst.未开工剩余货量_预计三年后开工剩余面积,
        rst.未开工剩余货量_预计三年后开工剩余货值,
        rst.未开工剩余货量_预计三年后开工剩余套数,
        ...
        rst.建设状态_已达形象未达竣备剩余面积,
        rst.建设状态_已达形象未达竣备剩余货值,
        rst.建设状态_已达形象未达竣备剩余套数,

        rst.建设状态_已开工未达形象剩余面积,
        rst.建设状态_已开工未达形象剩余货值,
        rst.建设状态_已开工未达形象剩余套数,
        ...
        rst.[管理方式]
    INTO #temp_result
    FROM [172.16.4.161].highdata_prod.dbo.data_wide_dws_s_WqBaseStatic_Organization org
    LEFT JOIN #result rst ON rst.SaleBldGUID = org.组织架构id
    WHERE org.组织架构类型 = 7
    ...
    select  
        ...
        null as [集中交付完成日期],
        NULL as [立项均价],
        sum(rst.[立项货值]) as [立项货值],
        null as [定位均价],
        sum(rst.[定位货值]) as [定位货值],
        sum(rst.[总建面]) as [总建面],
        sum(rst.[地上建面]) as [地上建面],
        sum(rst.[地下建面]) as [地下建面],
        sum(rst.[总可售面积]) as [总可售面积],
        sum(rst.[动态总货值]) as [动态总货值],
        null as [整盘均价],
        sum(rst.[已售面积]) as [已售面积],
        sum(rst.[已售货值]) as [已售货值],
        null as [已售均价],
        SUM(rst.[待售面积]) as [待售面积],
        SUM(rst.[待售货值]) as [待售货值],
        null as [预测单价],
        SUM(rst.[年初可售面积]) as [年初可售面积],
        SUM(rst.[年初可售货值]) as [年初可售货值],
        SUM(rst.[本年签约面积]) as [本年签约面积],
        SUM(rst.[本年签约金额]) as [本年签约金额],
        null as [本年签约均价],
        SUM(rst.[本月签约面积]) as [本月签约面积],
        SUM(rst.[本月签约金额]) as [本月签约金额],
        null as [本月签约均价],
        SUM(rst.[本年预计签约面积]) as [本年预计签约面积],
        SUM(rst.[本年预计签约金额]) as [本年预计签约金额],
        null as [正式开工实际完成时间],
        null as [正式开工预计完成时间],
        SUM(rst.[待售套数]) as [待售套数],
        SUM(rst.[总可售套数]) as [总可售套数],
        null as [首推时间],
        null as [业态组合键],
        null as [营业成本单方],
        null as [土地款单方],
        ...
        null as [股权溢价单方],
        null as [总成本不含税单方],
        sum(rst.[已售对应总成本]) as [已售对应总成本],
        sum(rst.[已售货值不含税]) as [已售货值不含税],
        sum(rst.[已售净利润签约]) as [已售净利润签约],
        sum(rst.[未售对应总成本]) as [未售对应总成本],
        null as [近三月签约金额均价不含税],
        null as [近六月签约金额均价不含税],
        null as [立项单价],
        null as [定位单价],
        null as [已售均价不含税],
        null as [货量铺排均价计算方式],
        sum(rst.[未售货值不含税]) as [未售货值不含税],

        sum(rst.[未售净利润签约]) as [未售净利润签约],
        sum(rst.[项目已售税前利润签约]) as [项目已售税前利润签约],
        sum(rst.[项目未售税前利润签约]) as [项目未售税前利润签约],
        sum(rst.[项目整盘利润]) as [项目整盘利润],
        null as [货值铺排均价不含税],


        sum(rst.[已售套数]) as [已售套数],
        sum(rst.[近三月签约金额不含税] ) as [近三月签约金额不含税],
        sum(rst.[近三月签约面积]) as [近三月签约面积],
        sum(rst.[近六月签约金额不含税]) as [近六月签约金额不含税],
        sum(rst.[近六月签约面积]) as [近六月签约面积],
        NULL as [ztguid],
        NULL as [是否停工],
        null as [项目首推时间],
        null as [首开楼栋标签],
        sum(rst.[首开30天签约套数]) as [首开30天签约套数],
        sum(rst.[首开30天签约面积]) as [首开30天签约面积],
        sum(rst.[首开30天签约金额]) as [首开30天签约金额],
        sum(rst.[首开30天认购套数]) as [首开30天认购套数],
        sum(rst.[首开30天认购面积]) as [首开30天认购面积],
        sum(rst.[首开30天认购金额]) as [首开30天认购金额],
        sum(rst.[总产值]) as [总产值],
        sum(rst.[已完成产值]) as [已完成产值],
        sum(rst.[待发生产值]) as [待发生产值],
        sum(rst.[合同约定应付金额]) as [合同约定应付金额],
        sum(rst.[累计支付金额]) as [累计支付金额],
        sum(rst.[产值未付]) as [产值未付],
        sum(rst.[应付未付]) as [应付未付],
        sum(rst.[持有套数]) as [持有套数],
        sum(rst.[持有面积]) as [持有面积],
        sum(rst.[土地分摊金额] ) as [土地分摊金额],
        sum(rst.[除地价外直投分摊金额]) as [除地价外直投分摊金额],
        sum(rst.[已发生营销费用摊分金额]) as [已发生营销费用摊分金额],
        sum(rst.[已发生管理费用摊分金额]) as [已发生管理费用摊分金额],
        sum(rst.[已发生财务费用费用摊分金额]) as [已发生财务费用费用摊分金额],
        sum(rst.[已发生税金分摊]) as [已发生税金分摊],
        sum(rst.[已结转套数]) as [已结转套数],
        sum(rst.[已结转面积]) as [已结转面积],
        sum(rst.[已结转收入]) as [已结转收入],
        sum(rst.[已结转成本]) as [已结转成本],
        sum(rst.[签约套数2024年]) as [签约套数2024年],
        sum(rst.[签约面积2024年]) as [签约面积2024年],
        sum(rst.[签约金额2024年]) as [签约金额2024年],
        sum(rst.[签约均价2024年]) as [签约均价2024年],
        null as  [经营成本单方],
        null as [赛道图标签],
        sum(rst.[占压资金]) as [占压资金],
        sum(rst.[累计回笼金额]) as [累计回笼金额],
        sum(rst.[累计本年回笼金额]) as [累计本年回笼金额],
        sum(rst.[累计本月回笼金额]) as [累计本月回笼金额],
        sum(rst.[用地面积]) as [用地面积],
        sum(rst.[可售面积]) as [可售面积],
        sum(rst.[计容面积]) as [计容面积],
        sum(rst.[自持面积]) as [自持面积],
        sum(rst.[自持可售面积]) as [自持可售面积],
        sum(rst.[地上可售面积]) as [地上可售面积],
        sum(rst.[地上自持面积]) as [地上自持面积],
        sum(rst.[地上可售面积地上自持面积]) as [地上可售面积地上自持面积],
        rst.[项目GUID],
        rst.[股权比例],
        rst.[所属城市],
        sum(rst.[持有货值]) as [持有货值],
        -- 新增字段
        sum(rst.[反算总货值_总货值]) as [反算总货值_总货值],
        sum(rst.[反算总货值_总面积]) as [反算总货值_总面积],
        sum(rst.反算总货值_套数) as [反算总货值_套数],
        sum(rst.[反算总货值_剩余货值]) as [反算总货值_剩余货值],
        sum(rst.[反算总货值_剩余面积]) as [反算总货值_剩余面积],
        sum(rst.[反算总货值_剩余套数] ) as [反算总货值_剩余套数],
        sum(rst.[已开工情况_已开工货值] ) as [已开工情况_已开工货值],
        sum(rst.[已开工情况_已开工面积]) as [已开工情况_已开工面积],
        sum(rst.[已开工情况_已开工套数]) as [已开工情况_已开工套数],
        sum(rst.[已开工情况_已开工已售货值]) as [已开工情况_已开工已售货值],
        sum(rst.[已开工情况_已开工已售面积]) as [已开工情况_已开工已售面积],
        sum(rst.[已开工情况_已开工已售套数]) as [已开工情况_已开工已售套数],     
        sum(rst.[已开工情况_已开工未售货值]) as [已开工情况_已开工未售货值],
        sum(rst.[已开工情况_已开工未售面积]) as [已开工情况_已开工未售面积],
        sum(rst.[已开工情况_已开工未售套数]) as [已开工情况_已开工未售套数],
         
        sum(rst.[存货情况_存货货值]) as [存货情况_存货货值],
        sum(rst.[存货情况_存货面积]) as [存货情况_存货面积],
        sum(rst.[存货情况_存货套数]) as [存货情况_存货套数],
        sum(rst.[自持情况_总自持资产货值] ) as [自持情况_总自持资产货值],
        sum(rst.[自持情况_总自持资产面积]) as [自持情况_总自持资产面积],
        sum(rst.[自持情况_总自持资产套数]) as [自持情况_总自持资产套数],
        sum(rst.[自持情况_已转经营货值]) as [自持情况_已转经营货值],
        sum(rst.[自持情况_已转经营面积]) as [自持情况_已转经营面积],
        sum(rst.[自持情况_已转经营套数]) as [自持情况_已转经营套数],     
        sum(rst.[在途情况_在途货值]  )as [在途情况_在途货值],
        sum(rst.[在途情况_在途面积]) as [在途情况_在途面积],
        sum(rst.[在途情况_在途套数]) as [在途情况_在途套数],
        sum(rst.[未开工情况_未开工货值]) as [未开工情况_未开工货值],
        sum(rst.[未开工情况_未开工面积]) as [未开工情况_未开工面积],
        sum(rst.[未开工情况_未开工套数]) as [未开工情况_未开工套数],
        sum(rst.[分货转经营面积]) as [分货转经营面积],
        sum(rst.[分货转经营金额]) as [分货转经营金额],
        sum(rst.[分货转经营套数]) as [分货转经营套数],
        sum(rst.[分货销售面积]) as [分货销售面积],
        sum(rst.[分货销售金额]) as [分货销售金额],
        sum(rst.[分货销售套数]) as [分货销售套数],

         -- 新增字段
        sum(rst.处置前临时冻结面积合计) as 处置前临时冻结面积合计,
        sum(rst.处置前临时冻结_合作受阻冻结面积) as 处置前临时冻结_合作受阻冻结面积,
        sum(rst.处置前临时冻结_开发受限冻结面积) as 处置前临时冻结_开发受限冻结面积,
        sum(rst.处置前临时冻结_停工缓建冻结面积) as 处置前临时冻结_停工缓建冻结面积,
        sum(rst.处置前临时冻结_投资未落实冻结面积) as 处置前临时冻结_投资未落实冻结面积,

        sum(rst.处置前临时冻结货值合计) as 处置前临时冻结货值合计,
        sum(rst.处置前临时冻结_合作受阻冻结货值) as 处置前临时冻结_合作受阻冻结货值,
        sum(rst.处置前临时冻结_开发受限冻结货值) as 处置前临时冻结_开发受限冻结货值,
        sum(rst.处置前临时冻结_停工缓建冻结货值) as 处置前临时冻结_停工缓建冻结货值,
        sum(rst.处置前临时冻结_投资未落实冻结货值) as 处置前临时冻结_投资未落实冻结货值,

        sum(rst.处置前临时冻结套数合计) as 处置前临时冻结套数合计,
        sum(rst.处置前临时冻结_合作受阻冻结套数) as 处置前临时冻结_合作受阻冻结套数,
        sum(rst.处置前临时冻结_开发受限冻结套数) as 处置前临时冻结_开发受限冻结套数,
        sum(rst.处置前临时冻结_停工缓建冻结套数) as 处置前临时冻结_停工缓建冻结套数,
        sum(rst.处置前临时冻结_投资未落实冻结套数) as 处置前临时冻结_投资未落实冻结套数,

        sum(rst.处置前退换调转剩余面积) as 处置前退换调转剩余面积,
        sum(rst.处置前退换调转剩余货值) as 处置前退换调转剩余货值,
        sum(rst.处置前退换调转剩余套数) as 处置前退换调转剩余套数,

        sum(rst.处置前正常销售剩余面积) as 处置前正常销售剩余面积,
        sum(rst.处置前正常销售剩余货值) as 处置前正常销售剩余货值,
        sum(rst.处置前正常销售剩余套数) as 处置前正常销售剩余套数,

        sum(rst.处置前转经营剩余面积) as 处置前转经营剩余面积,
        sum(rst.处置前转经营剩余货值) as 处置前转经营剩余货值,
        sum(rst.处置前转经营剩余套数) as 处置前转经营剩余套数,

        sum(rst.处置前尾盘剩余面积) as 处置前尾盘剩余面积,
        sum(rst.处置前尾盘剩余货值) as 处置前尾盘剩余货值,
        sum(rst.处置前尾盘剩余套数) as 处置前尾盘剩余套数,

        -- 处置后
        sum(rst.处置后临时冻结面积合计) as 处置后临时冻结面积合计,
        sum(rst.处置后临时冻结_合作受阻冻结面积) as 处置后临时冻结_合作受阻冻结面积,
        sum(rst.处置后临时冻结_开发受限冻结面积) as 处置后临时冻结_开发受限冻结面积,
        sum(rst.处置后临时冻结_停工缓建冻结面积) as 处置后临时冻结_停工缓建冻结面积,
        sum(rst.处置后临时冻结_投资未落实冻结面积) as 处置后临时冻结_投资未落实冻结面积,

        sum(rst.处置后临时冻结货值合计) as 处置后临时冻结货值合计,
        sum(rst.处置后临时冻结_合作受阻冻结货值) as 处置后临时冻结_合作受阻冻结货值 ,
        sum(rst.处置后临时冻结_开发受限冻结货值) as 处置后临时冻结_开发受限冻结货值 ,
        sum(rst.处置后临时冻结_停工缓建冻结货值) as 处置后临时冻结_停工缓建冻结货值 ,
        sum(rst.处置后临时冻结_投资未落实冻结货值) as 处置后临时冻结_投资未落实冻结货值,

        sum(rst.处置后临时冻结套数合计) as 处置后临时冻结套数合计,
        sum(rst.处置后临时冻结_合作受阻冻结套数) as 处置后临时冻结_合作受阻冻结套数,
        sum(rst.处置后临时冻结_开发受限冻结套数) as 处置后临时冻结_开发受限冻结套数,
        sum(rst.处置后临时冻结_停工缓建冻结套数) as 处置后临时冻结_停工缓建冻结套数 ,
        sum(rst.处置后临时冻结_投资未落实冻结套数) as 处置后临时冻结_投资未落实冻结套数,

        sum(rst.处置后退换调转剩余面积) as 处置后退换调转剩余面积,
        sum(rst.处置后退换调转剩余货值) as 处置后退换调转剩余货值,
        sum(rst.处置后退换调转剩余套数) as 处置后退换调转剩余套数,

        sum(rst.处置后正常销售剩余面积) as 处置后正常销售剩余面积 ,
        sum(rst.处置后正常销售剩余货值) as 处置后正常销售剩余货值,
        sum(rst.处置后正常销售剩余套数) as 处置后正常销售剩余套数,

        sum(rst.处置后转经营剩余面积) as 处置后转经营剩余面积,
        sum(rst.处置后转经营剩余货值) as 处置后转经营剩余货值,
        sum(rst.处置后转经营剩余套数) as 处置后转经营剩余套数,

        sum(rst.处置后尾盘剩余面积) as 处置后尾盘剩余面积,
        sum(rst.处置后尾盘剩余货值) as 处置后尾盘剩余货值,
        sum(rst.处置后尾盘剩余套数) as 处置后尾盘剩余套数,


        -- 新增字段
        sum(rst.已开工剩余货量_已推未售剩余面积) as 已开工剩余货量_已推未售剩余面积,
        sum(rst.已开工剩余货量_已推未售剩余货值) as 已开工剩余货量_已推未售剩余货值,
        sum(rst.已开工剩余货量_已推未售剩余套数) as 已开工剩余货量_已推未售剩余套数,

        sum(rst.已开工剩余货量_获证待推剩余面积) as 已开工剩余货量_获证待推剩余面积,
        sum(rst.已开工剩余货量_获证待推剩余货值) as 已开工剩余货量_获证待推剩余货值,
        sum(rst.已开工剩余货量_获证待推剩余套数) as 已开工剩余货量_获证待推剩余套数,
        
        sum(rst.已开工剩余货量_达形象未取证剩余面积) as 已开工剩余货量_达形象未取证剩余面积,
        sum(rst.已开工剩余货量_达形象未取证剩余货值) as 已开工剩余货量_达形象未取证剩余货值,
        sum(rst.已开工剩余货量_达形象未取证剩余套数) as 已开工剩余货量_达形象未取证剩余套数,
         
        sum(rst.已开工剩余货量_正常在途剩余面积) as 已开工剩余货量_正常在途剩余面积,
        sum(rst.已开工剩余货量_正常在途剩余货值) as 已开工剩余货量_正常在途剩余货值,
        sum(rst.已开工剩余货量_正常在途剩余套数) as 已开工剩余货量_正常在途剩余套数,

        sum(rst.已开工剩余货量_停工缓建在途剩余面积) as 已开工剩余货量_停工缓建在途剩余面积,
        sum(rst.已开工剩余货量_停工缓建在途剩余货值) as 已开工剩余货量_停工缓建在途剩余货值,
        sum(rst.已开工剩余货量_停工缓建在途剩余套数) as 已开工剩余货量_停工缓建在途剩余套数,

        sum(rst.未开工剩余货量_停工缓建剩余面积) as 未开工剩余货量_停工缓建剩余面积,
        sum(rst.未开工剩余货量_停工缓建剩余货值) as 未开工剩余货量_停工缓建剩余货值,
        sum(rst.未开工剩余货量_停工缓建剩余套数) as 未开工剩余货量_停工缓建剩余套数,

        sum(rst.未开工剩余货量_预计三年内开工剩余面积) as 未开工剩余货量_预计三年内开工剩余面积,
        sum(rst.未开工剩余货量_预计三年内开工剩余货值) as 未开工剩余货量_预计三年内开工剩余货值,
        sum(rst.未开工剩余货量_预计三年内开工剩余套数) as 未开工剩余货量_预计三年内开工剩余套数,

        sum(rst.未开工剩余货量_预计三年后开工剩余面积) as 未开工剩余货量_预计三年后开工剩余面积,
        sum(rst.未开工剩余货量_预计三年后开工剩余货值) as 未开工剩余货量_预计三年后开工剩余货值,
        sum(rst.未开工剩余货量_预计三年后开工剩余套数) as 未开工剩余货量_预计三年后开工剩余套数,

        sum(rst.建设状态_已竣备剩余面积) as 建设状态_已竣备剩余面积,
        sum(rst.建设状态_已竣备剩余货值) as 建设状态_已竣备剩余货值,
        sum(rst.建设状态_已竣备剩余套数) as 建设状态_已竣备剩余套数,

        sum(rst.建设状态_已达形象未达竣备剩余面积) as 建设状态_已达形象未达竣备剩余面积,
        sum(rst.建设状态_已达形象未达竣备剩余货值) as 建设状态_已达形象未达竣备剩余货值,
        sum(rst.建设状态_已达形象未达竣备剩余套数) as 建设状态_已达形象未达竣备剩余套数,

        sum(rst.建设状态_已开工未达形象剩余面积) as 建设状态_已开工未达形象剩余面积,
        sum(rst.建设状态_已开工未达形象剩余货值) as 建设状态_已开工未达形象剩余货值,
        sum(rst.建设状态_已开工未达形象剩余套数) as 建设状态_已开工未达形象剩余套数,

        sum(rst.建设状态_停工缓建剩余面积) as 建设状态_停工缓建剩余面积,
        sum(rst.建设状态_停工缓建剩余货值) as 建设状态_停工缓建剩余货值,
        sum(rst.建设状态_停工缓建剩余套数) as 建设状态_停工缓建剩余套数,
        ...
        null as [管理方式]
    from  [172.16.4.161].highdata_prod.dbo.data_wide_dws_s_WqBaseStatic_Organization org
    LEFT JOIN #result rst ON rst.项目GUID = org.组织架构id
    where  org.组织架构类型 =3
    group by  
        org.组织架构ID,
        org.组织架构名称,
...
BEGIN 
    ...
    SELECT 
        ...
        max(rst.[OrgGUID]) as [OrgGUID],
        max(rst.[平台公司]) as [平台公司],
        case when o.组织架构类型 >=3 then max(rst.[项目名称]) else null  end as  [项目名称],
        case when o.组织架构类型 >=3 then max(rst.[项目推广名]) else null  end as  [项目推广名],
        case when o.组织架构类型 >=3 then max(rst.[明源系统代码]) else null  end as [明源系统代码],
        case when o.组织架构类型 >=3 then max(rst.[项目代码]) else null  end as [项目代码],
        case when o.组织架构类型 >=3 then max(rst.[获取时间]) else null  end as [获取时间],
        case when o.组织架构类型 >=3 then max(rst.[总地价]) else null  end as [总地价],
        case when o.组织架构类型 >=3 then max(rst.[是否合作项目]) else null  end as [是否合作项目],
        null as [分期名称],
        null as [产品楼栋名称],
        null as [SaleBldGUID],
        null as [GCBldGUID],
        null as [工程楼栋名称],
        case when o.组织架构类型 >=4 then max(rst.[产品类型]) else null  end as [产品类型],
        null as [产品名称],
        null as [商品类型],
        null as [是否可售],
        null as [是否持有],
        ...
        sum(rst.[地上自持面积]) as [地上自持面积],
        sum(rst.[地上可售面积地上自持面积]) as [地上可售面积地上自持面积],
        case when o.组织架构类型 >=3 then max(rst.[项目GUID]) else null  end as [项目GUID],
        case when o.组织架构类型 >=3 then max(rst.[股权比例]) else null  end as [股权比例],
        case when o.组织架构类型 >=3 then max(rst.[所属城市]) else null  end as [所属城市],
        sum(rst.[持有货值]) as [持有货值],
         -- 新增字段
        sum(rst.[反算总货值_总货值]) as [反算总货值_总货值],
        sum(rst.[反算总货值_总面积]) as [反算总货值_总面积],
    ...
    FROM 
        ...
        INNER JOIN [172.16.4.161].highdata_prod.dbo.data_wide_dws_s_WqBaseStatic_Organization o1 ON o1.组织架构父级id = o.组织架构id
        LEFT JOIN #temp_result rst ON rst.组织架构id = o1.组织架构id  
    WHERE 
        o.组织架构类型 = @baseinfo
    GROUP BY 
