USE [HighData_prod]
GO
/****** Object:  StoredProcedure [dbo].[usp_rpt_wqzydtBi]    Script Date: 2025/5/5 17:36:10 ******/
SET ANSI_NULLS ON
GO
...
da.时间,
sum(case when da.时间 = '累计' then org.累计经营性现金流 when da.时间 = '本年' then org.本年经营性现金流 else  本月经营性现金流 end) as 经营性现金流,	
sum(case when da.时间 = '累计' then org.累计现金流入 when da.时间 = '本年' then org.本年现金流入 else  本月现金流入 end)  现金流入,	
sum(case when da.时间 = '累计' then org.累计现金流出 when da.时间 = '本年' then org.本年现金流出 else  本月现金流出 end) 现金流出,	
sum(case when da.时间 = '累计' then org.累计地价支出 when da.时间 = '本年' then org.本年地价支出 else  本月地价支出 end) 地价,	
sum(case when da.时间 = '累计' then org.累计除地价外直投发生 when da.时间 = '本年' then org.本年除地价外直投发生 else  本月除地价外直投发生 end) 直投,	
sum(case when da.时间 = '累计' then org.累计费用发生 when da.时间 = '本年' then org.本年费用发生 else  本月费用发生 end) 费用,	
sum(case when da.时间 = '累计' then org.累计税金支出 when da.时间 = '本年' then org.本年税金支出 else  本月税金支出 end) 税金,	
sum(case when da.时间 = '累计' then org.累计贷款余额 when da.时间 = '本年' then org.本年净增贷款 else  本月贷款金额 end) 贷款,	
sum(case when da.时间 = '累计' then org.累计股东现金流 when da.时间 = '本年' then org.本年股东现金流 else  本月股东现金流 end) 股东现金流
from s_WqBaseStatic_summary org
...
--本年
,case when sum(lr.本年已签约面积) = 0 then 0 else sum(case when 业态 = '地下室/车库' then lr.本年已签约金额*10000 else lr.本年已签约金额 end)*10000.0/sum(lr.本年已签约面积) end  本年签约单价
,sum(lr.本年已签约面积) as 本年签约面积 	
