...
SELECT  r.projguid ,
        ...
        SUM(r.bldarea) zksmj ,
		sum(case when r.hszj = 0 then isnull(jd.ycprice,0)*r.bldarea else r.hszj end )hszj,
        COUNT(1) zksts ,
        --认购口径
        ...
        --近三月认购情况
        SUM(CASE WHEN r.status in ('签约','认购') AND   DATEDIFF(mm, so.qsdate, GETDATE()) between 1 and 3 
        THEN isnull(sc.JyTotal,so.jytotal) ELSE 0 END) rgje_j3y ,
        SUM(CASE WHEN r.status in ('签约','认购') AND   DATEDIFF(mm, so.qsdate, GETDATE()) between 1 and 3 THEN r.bldarea ELSE 0 END) rgmj_j3y ,
        SUM(CASE WHEN r.status in ('签约','认购') AND   DATEDIFF(mm, so.qsdate, GETDATE()) between 1 and 3 THEN 1 ELSE 0 END) rgts_j3y ,
        ...
        --近三月签约情况
        SUM(CASE WHEN r.status = '签约' AND   DATEDIFF(mm, sc.qsdate, GETDATE()) between 1 and 3 THEN sc.JyTotal ELSE 0 END) qyje_j3y ,
        SUM(CASE WHEN r.status = '签约' AND   DATEDIFF(mm, sc.qsdate, GETDATE()) between 1 and 3 THEN r.bldarea ELSE 0 END) qymj_j3y ,
        SUM(CASE WHEN r.status = '签约' AND   DATEDIFF(mm, sc.qsdate, GETDATE()) between 1 and 3 THEN 1 ELSE 0 END) qyts_j3y ,
		...
		sum(case when DATEDIFF(dd, thdate, GETDATE()) >= 0 and r.hszj = 0 then isnull(jd.ycprice,0)*r.bldarea when DATEDIFF(dd, thdate, GETDATE()) >= 0 then r.hszj else 0 end)ythz,--已推货值
...
SELECT  ld.projguid ,
		...
		sum(case when DATEDIFF(dd, ld.SJkpxsDate, GETDATE()) >= 0 then ISNULL(hx.HxArea, 0) * ISNULL(hx.HgNum, 0)*isnull(ld.ycprice,0) else 0 end)ythz,--已推货值
...
FROM    MyCost_Erp352..vmd_ProductBuild_ZX t
        LEFT JOIN MyCost_Erp352..md_Room r ON r.ProductBldGUID = t.productbuildguid
        INNER JOIN MyCost_Erp352..md_ProductBuildHxAreaHgNumSetting hx ON t.ProductBuildGUID = hx.ProductBuildGUID AND t.versionguid = hx.versionguid
...
WHERE   t.HxAreaHgNumSettingGUID is null and NOT EXISTS (SELECT  *
                    FROM    #tmp tmp
                    WHERE   t.productbuildguid = tmp.productbuildguid);  
