USE [ERP25]
GO
/****** Object:  StoredProcedure [dbo].[usp_ydkb_dthz_wq_deal_cbinfo]    Script Date: 2025/1/6 10:37:09 ******/
SET ANSI_NULLS ON
GO
   ...
   from [172.16.4.161].[HighData_prod].dbo.data_tb_Wq_LowerCostTask
    ...
    (
      ...
      SELECT bi2.组织架构父级ID,
          ...
          NULL AS 地价,
          -- 已发生成本
          isnull(已发生成本除地价外直投,0)+isnull(已发生成本土地款,0)+isnull(动态成本营销费用,0)+isnull(动态成本管理费用,0)+isnull(动态成本财务费用,0) 已发生成本,
          已发生成本直投,
          已发生成本除地价外直投,
          已发生成本土地款,
          已发生成本营销费用,
