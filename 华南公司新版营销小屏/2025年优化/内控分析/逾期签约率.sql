...
SELECT *,
    ...
    */
    CASE 
        WHEN 总认购套数 > 0 AND 逾期签约率 <= 0.2 THEN 
            -- 红榜评分
            CASE 
                WHEN 逾期签约率 <= 0.2 AND 逾期签约率 > 0.05 THEN '及格'
                WHEN 逾期签约率 <= 0.05 AND 逾期签约率 > 0 THEN '良好'
        ...
        ELSE  
            ...
            CASE  
                ...
                WHEN 逾期签约率 > 0.2 AND 逾期签约率 <= 0.3 THEN '不及格'
            END 
    END AS 评分,
    CASE 
        WHEN 总认购套数 > 0 AND 逾期签约率 <= 0.2 THEN '红榜' 
        ELSE '黑榜' 
    END 红黑榜
INTO #ztfl
FROM (
...
FROM (
    SELECT 
        *,
        rank() OVER(ORDER BY 红黑榜 DESC, 逾期签约率) AS 正序,
        rank() OVER(ORDER BY 红黑榜, 逾期签约率 DESC) AS 倒序
    FROM #ztfl 
) t
WHERE 1=1
