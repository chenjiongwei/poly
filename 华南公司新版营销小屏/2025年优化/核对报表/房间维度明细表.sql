--  华南公司房间维度明细表
DECLARE @var_Sdate DATETIME = DATEADD(yy, DATEDIFF(yy,0,getdate()), 0)  
...
SELECT  BuildingGUID ,
        ...
        PlanFinishDate, -- 竣备备案计划完成日期
        ExpectedFinishDate -- 竣工备案预计完成日期
INTO    #Bld
...
select *
into #dhx
from [172.16.4.161].[HighData_prod].dbo.data_tb_hnyx_areasection mjd;
...
FROM(SELECT p2.ProjCode 明源项目代码 ,
            ...
            bld.PlanFinishDate as 计划竣工备案日期,
            o.IsSzyx AS 是否数字营销,
     ...
     FROM   #order o
			...
			LEFT JOIN #顶层 dc ON dc.projGUID = r.projGUID AND   dc.bldGUID = r.bldGUID
     		LEFT JOIN #dhx dhx ON r.BldArea between dhx.开始面积 and dhx.截止面积 and dhx.业态 = bld.TopProductTypeName
