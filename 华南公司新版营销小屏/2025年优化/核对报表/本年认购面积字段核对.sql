    ...
    -- 取特殊业绩和合作业绩
      --获取各产品业态的特殊业绩及合作项目的认购情况
        ...
        LEFT JOIN(
                    ...
                    SELECT s.ParentProjGUID AS projguid ,
                        ...
                        AND  ( DATEDIFF(YEAR, StatisticalDate, @var_date) = 0 OR (s.TsyjType = '物业公司车位代销' AND   DATEDIFF(YEAR, StatisticalDate, @var_date) = 0) ) THEN CCjAmount ELSE 0 END) AS zbnccphzje ,    --准产成品本年认购金额
        ...
        WHERE   pj.Level = 2 --and     pj.projguid ='7f0a49da-5a96-e911-80b7-0a94ef7517dd'
        GROUP BY pj.ProjGUID ,
                 pt.TopProductTypeName;
        ...
        SELECT  p.projguid ,
                bld.TopProductTypeName ,
                SUM(CASE WHEN YEAR(ISNULL(qt.认定日期,a.StatisticalDate)) = YEAR(@var_date) and  a.TsyjType <> '物业公司车位代销' THEN r.CjRmbTotal ELSE 0 END) / 10000.0 AS 其他业绩本年认购金额 ,
                SUM(CASE WHEN YEAR(ISNULL(qt.认定日期,a.StatisticalDate)) = YEAR(@var_date) and  a.TsyjType <> '物业公司车位代销' THEN r.CjBldArea ELSE 0 END) AS 其他业绩本年认购面积 ,
                SUM(CASE WHEN YEAR(ISNULL(qt.认定日期,a.StatisticalDate)) = YEAR(@var_date) and  a.TsyjType <> '物业公司车位代销' THEN 1 ELSE 0 END) AS 其他业绩本年认购套数
        INTO    #qtqy
        ...
        LEFT JOIN data_tb_hnyx_areasection mjd ON r.BldArea between mjd.开始面积 and mjd.截止面积 and mjd.业态 = bld.TopProductTypeName
        LEFT JOIN data_wide_s_LdfSaleDtl ldf on ldf.RoomGUID =r.RoomGUID
        WHERE 1 = 1 and  a.SetGqAuditTime is null --and  p.projguid ='7f0a49da-5a96-e911-80b7-0a94ef7517dd' -- DATEDIFF(DAY, a.[StatisticalDate], qt.认定日期) = 0 AND 
      ...
      select 
		...
		WHERE   Level = 2 AND   p.buguid = '70DD6DF4-47F7-46AF-B470-BC18EE57D8FF'  and tb.投管编码 ='2954' 
