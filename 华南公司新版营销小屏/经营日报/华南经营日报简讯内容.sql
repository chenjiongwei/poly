...
SELECT 
    ...
    CASE 
        WHEN SUM(本月签约金额不含税) = 0 THEN 0 
        ELSE SUM(本月净利润签约) / SUM(本月签约金额不含税) 
    END AS 本月签约销净率,
    SUM(本年签约金额) AS 本年签约金额,
    SUM(本年签约金额不含税) AS 本年签约金额不含税,
    SUM(本年净利润签约) AS 本年净利润签约,
    CASE 
        WHEN SUM(本年签约金额不含税) = 0 THEN 0 
        ELSE SUM(本年净利润签约) / SUM(本年签约金额不含税) 
    END AS 本年签约销净率
INTO #jlr
...
WITH ProjectNetRateRanking AS (
    SELECT 
        ...
        END AS 认购销净率,
        SUM(本日签约金额) * 10000 AS 本日签约金额,
        CASE 
            WHEN SUM(本日签约金额不含税) = 0 THEN 0 
            ELSE SUM(本日净利润签约) / SUM(本日签约金额不含税) 
        END AS 签约销净率
    FROM s_M002项目级毛利净利汇总表New t WITH (NOLOCK)
...
SELECT 
    ...
    本日签约金额,
    签约销净率,
    ROW_NUMBER() OVER(ORDER BY 签约销净率 DESC) AS 排名_前,
    ROW_NUMBER() OVER(ORDER BY 签约销净率) AS 排名_后
INTO #jlr_rn
...
SELECT 
    所属区域, 
    STUFF((
        SELECT CHAR(10) + '【' + t1.SpreadName + '】今日签约' + t1.业态 + 
               -- CONVERT(VARCHAR(10), CONVERT(DECIMAL(16, 2), 本日认购金额)) + '万,认购销净率' +
               -- CONVERT(VARCHAR(10), CONVERT(DECIMAL(16, 2), 认购销净率 * 100)) + '%' +
               -- '，签约' +
               CONVERT(VARCHAR(10), CONVERT(DECIMAL(16, 2), 本日签约金额)) + '万,销净率' +
               CONVERT(VARCHAR(10), CONVERT(DECIMAL(16, 2), 签约销净率 * 100)) + '%'
        FROM #jlr_rn t1 
        WHERE (t1.排名_前 BETWEEN 1 AND 2 OR t1.排名_后 BETWEEN 1 AND 2)
        AND t1.所属区域 = t.所属区域
        ORDER BY t1.签约销净率 DESC
        FOR XML PATH('')
    ), 1, 1, '') AS 典型项目去化
INTO #jlr_result
FROM #jlr_rn t
...
SELECT 
    ...
    CONVERT(VARCHAR(10), CONVERT(DECIMAL(16, 2), ISNULL(a.本月签约销净率 * 100, 0))) + '%' AS 本月签约销净率,
    CONVERT(VARCHAR(10), CONVERT(DECIMAL(16, 2), ISNULL(a.本年签约销净率 * 100, 0))) + '%' AS 本年签约销净率,
    a.本年存量项目认购金额,
    a.本年增量项目认购金额,
    a.本年新增量项目认购金额,
    a.佛山本年认购金额,
