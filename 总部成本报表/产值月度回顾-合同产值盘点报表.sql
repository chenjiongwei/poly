...
SELECT  
        ...
        ovr.OutputValueMonthReviewName AS 回顾名称,
        ...
        BldName AS 楼栋名称,
        isnull(a.BldArea,0) AS 建筑面积,
        a.Jszt AS 建设状态,
        isnull(a.Xmpdljwccz,0) AS [项目盘点累计完成产值G],
        isnull(a.Ljyfkje,0) AS [项目盘点累计应付款I],
        isnull(a.Dfscz,0) AS [待发生产值H],
        isnull(a.LjsfkNoFxj,0) AS [累计实付款不含非现金],
        isnull(a.Ljsfk,0) AS [累计实付款],
        ISNULL(a.Xmpdljwccz, 0) - ISNULL(a.LjsfkNoFxj, 0) AS [已发产值未付金额K],
        ISNULL(a.Ljyfkje, 0) - ISNULL(a.LjsfkNoFxj, 0) AS [应付未付金额L],
        isnull(a.Sgdwysbcz,0) AS [施工单位已申报产值],
        isnull(a.Sgdwysbyfk,0) AS [施工单位已申报应付款],
        isnull(a.Jfysdcz,0) AS [甲方已审定产值],
        isnull(a.Jfysdyfk,0) AS [甲方已审定应付款]
...
FROM    cb_OutputValueMonthReview ovr
        INNER JOIN dbo.cb_OutputValueReviewDetail b ON ovr.OutputValueMonthReviewGUID = b.OutputValueMonthReviewGUID
        INNER JOIN dbo.cb_OutputValueReviewBld a ON b.OutputValueReviewDetailGUID = a.OutputValueReviewDetailGUID
        INNER JOIN p_Project p ON p.ProjGUID = ovr.ProjGUID
        INNER JOIN myBusinessUnit bu  ON bu.BUGUID = p.BUGUID
-- WHERE   b.OutputValueMonthReviewGUID = '2f2b0912-0f67-4d52-ac0e-c8b6b0b46a02'
WHERE p.projguid in ( @var_projguid )
and  FORMAT(ovr.ReviewDate, 'MM') in (@var_month) 
ORDER BY bu.BUName,
         p.ProjName,
