...
SELECT  pp.projguid , 
        ...
        END RoomNum ,    
        --签约情况
        SUM(case when sc.qsdate between @sdate and @edate then sc.JyTotal else 0 end) qyje ,
        SUM(case when sc.qsdate between @sdate and @edate then r.bldarea else 0 end ) qymj ,
        SUM(case when sc.qsdate between @sdate and @edate then 1 else 0 end ) qyts ,
        SUM(case when r.thdate is null then 0 else 1 end) ljtsts, -- 累计推售套数
        SUM(case when sc.qsdate is null then 0 else 1 end) ljqhts,--累计去化套数
...
select hx.projguid,
    ...
    sum(hx.ykgwsts) 已开工未售套数合计,
    sum(hx.ykgwsmj) 已开工未售建筑面积合计,
    sum(hx.ykgwshz) 已开工未售货值总计,
    sum(case when hx.SJzskgdate is not null and hx.SjDdysxxDate is null then hx.symj else 0 end) 在途货量面积合计, --已开工未达预售形象
    sum(case when hx.SJzskgdate is not null and hx.SjDdysxxDate is null then hx.syhz else 0 end) 在途货值合计,--剩余货值总计
    sum(hx.ykgwsmj - case when hx.SJzskgdate is not null and hx.SjDdysxxDate is null then hx.symj else 0 end) 存货面积合计, --已开工未售建筑面积合计-在途货量面积合计
    sum(hx.ykgwshz - case when hx.SJzskgdate is not null and hx.SjDdysxxDate is null then hx.syhz else 0 end) 存货货值合计,
    null as 在途货量达售时间
into #hl 
from [172.16.4.161].highdata_prod.dbo.data_wide_dws_s_HxAnalysis_Summary hx
inner join p_lddb ld on ld.salebldguid = hx.productbuildguid and datediff(dd,qxdate,getdate()) = 0
inner join #areatype t on (hx.producttype = t.producttype and (isnull(t.productname,'') =ld.productname  or t.productname is null) )  and hx.bldarea >= t.beginarea and hx.bldarea<t.endarea
inner join #p pj on pj.projguid = hx.projguid 
where hx.ykgwsts <> 0 
...
with s as (
    ...
    case when s.producttype = '地下室/车库' and sum(qyts) <> 0 then sum(qyje)/sum(qyts) when sum(qymj) = 0 then 0 else sum(qyje)/sum(qymj) end 月均去化均价,
    case when tj_day = 0 then 0 else sum(qymj)/tj_day*30 end 月均去化面积,
    case when tj_day = 0 then 0 else sum(qyts)*1.0/tj_day*30 end 月均去化套数,
    case when tj_day = 0 then 0 else sum(qyje)/tj_day*30 end 月均去化金额,
...
from (
        ...
        from #hl hl 
        left join s on s.projguid = hl.projguid and hl.业态 = s.producttype and isnull(hl.面积段分类,'') = isnull(s.areatype,'') and isnull(hl.户数,'') = isnull(s.户数,'')
        left join (
            SELECT  
                projguid,
                产品类型,
