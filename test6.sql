      declare @var_date date = '2025-07-28';
    ...
    -- 取特殊业绩和合作业绩
      --获取各产品业态的特殊业绩及合作项目的认购情况
        ...
        SELECT  p.projguid ,
                bld.TopProductTypeName ,
                SUM(CASE WHEN YEAR(ISNULL(qt.认定日期,a.StatisticalDate)) = YEAR(@var_date) THEN r.CjRmbTotal ELSE 0 END) / 10000.0 AS 其他业绩本年认购金额 ,
                SUM(CASE WHEN YEAR(ISNULL(qt.认定日期,a.StatisticalDate)) = YEAR(@var_date) THEN r.CjBldArea ELSE 0 END) AS 其他业绩本年认购面积 ,
                SUM(CASE WHEN YEAR(ISNULL(qt.认定日期,a.StatisticalDate)) = YEAR(@var_date) THEN 1 ELSE 0 END) AS 其他业绩本年认购套数 
        INTO    #qtqy
        ...
        LEFT JOIN data_tb_hnyx_areasection mjd ON r.BldArea between mjd.开始面积 and mjd.截止面积 and mjd.业态 = bld.TopProductTypeName
        LEFT JOIN data_wide_s_LdfSaleDtl ldf on ldf.RoomGUID =r.RoomGUID
        WHERE 1 = 1 --and  p.projguid ='7f0a49da-5a96-e911-80b7-0a94ef7517dd' -- DATEDIFF(DAY, a.[StatisticalDate], qt.认定日期) = 0 AND 
